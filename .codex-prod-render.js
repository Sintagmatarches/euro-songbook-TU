...
function historicalAffiliationSelectOptions(lang, groupKey = "", currentCountry = "") {
  ...
  const options = safeLang || selectedCountry
  ...
  return [
    ...
    ...options.map((opt) => `<option value="${esc(opt.value)}" ${selectedCountry === opt.value ? "selected" : ""}>${esc(opt.label)}</option>`),
  ].join("");
}
...
function normalizeCountryBackground(item, country, options = {}) {
  ...
  const visualProfile = normalizeVisualProfile(
    item?.visual_profile_json
    || item?.visual_profile
    || buildVisualProfileFromLegacyFields({
      desktop_image_value: item?.desktop_image_value,
      desktop_image_url: item?.desktop_image_url || item?.image_url || fallback?.desktop_image_url || "",
...
function listenSectionTitle() {
  ...
  if (uiLocale() === "uk") return "Слухати на платформах";
...
function homeLanguagePeriodsTitle(langLabel, countryLabel) {
  if (uiLocale() === "ru") return `${langLabel}: ${countryLabel} - СЌРїРѕС…Рё`;
  if (uiLocale() === "uk") return `${langLabel}: ${countryLabel} - РµРїРѕС…Рё`;
  if (uiLocale() === "et") return `${langLabel}: ${countryLabel} ajastud`;
  return `${langLabel}: ${countryLabel} eras`;
}

function homeLanguagePeriodsHint() {
  if (uiLocale() === "ru") return "Р’С‹Р±РµСЂРёС‚Рµ СЌРїРѕС…Сѓ.";
  if (uiLocale() === "uk") return "РћР±РµСЂС–С‚СЊ РµРїРѕС…Сѓ.";
...
function buildHomeSongSecondaryPreview(song = {}, options = {}) {
  ...
  const normalizePreviewLine = (value = "") => String(value || "").replace(/\s+/g, " ").trim();
  ...
  return collectPreviewLines(splitLyricsLines(String(song?.lyrics || "")));
}
...
function renderHomeSongCard(song = {}, options = {}) {
  ...
  const flagStyle = flagUrl ? ` style="${esc(`--yt-card-flag-image:${toCssUrlValue(flagUrl)}`)}"` : "";
  const yearValue = isPickerCard ? "" : String(song?.year || "").trim();
  const langValue = isPickerCard ? "" : String(song?.lang ? formatLang(song.lang) : "").trim();
  const countryValue = isPickerCard ? "" : String(song?.country ? formatCountry(song.country) : "").trim();
  const periodValue = isPickerCard ? "" : String(song?.period ? formatPeriod(song.period) : "").trim();
  const mainMetaParts = [
    ...
    periodValue ? `<span class="yt-card-main-meta-part yt-card-main-meta-soft">${esc(periodValue)}</span>` : "",
  ].filter(Boolean);
  ...
  */
  ...
  const cardClassName = ["yt-card", hasFlag ? "yt-card-has-flag" : "", `yt-card-preview-${previewLineLimit}`, extraClass]
    .filter(Boolean)
    .join(" ");
  return `
    <a class="${esc(cardClassName)}" href="${esc(href)}">
...
function renderHomeResultsFeedItems(items = [], options = {}) {
  ...
  if (!showGroupTitles) {
    return items.map((song, index) => `
      <div class="home-result-item" style="--result-index:${Math.min(index, 6)}">${renderHomeSongCard(song, options)}</div>
    `).join("");
  }
  const groups = [];
  ...
  return groups.map((group) => `
    <section class="home-result-group" data-bucket="${esc(group.bucket)}">
      ...
      ${group.items.map((song, index) => `<div class="home-result-item" style="--result-index:${Math.min(index, 6)}">${renderHomeSongCard(song, options)}</div>`).join("")}
    </section>
  `).join("");
}
    ...
    <section class="yt-main home-main ${shouldShowResults ? "is-search-results" : ""}">
      <div class="yt-chips ${showAdvanced ? "is-open" : "hidden"}" id="yt_advancedPanel">
        ...
        <label class="yt-chip-input">
          ...
          <select class="select" id="yt_country">${historicalAffiliationSelectOptions(lang || countrySelection.lang || "", "", country || "")}</select>
        </label>
        `}
        <label class="yt-chip-input" id="yt_period_wrap">
          <span>${esc(`${t("field.period")} (${uiText("legacyValue")})`)}</span>
        ...
        <label class="yt-chip-input">
          <span>${esc(uiLocale() === "ru" ? "Тематика" : uiLocale() === "uk" ? "Тематика" : uiLocale() === "et" ? "Temaatika" : "Theme")}</span>
...
function extractLyricsRenderLines(rawLyrics, lyricsMetaInput) {
  ...
  return sourceLines.map((text, index) => {
    ...
    const active = draftActiveVariant(line || {});
    const activeText = String(active?.text || text || "");
    ...
    return {
      raw_index: index,
      line_id: String(line?.id || `view_line_${index + 1}`),
      text: activeText,
      confidence,
...
function songDetailsUI(song, extra = {}) {
  ...
  const playActionBtn = hasListenItems
    ? `<button class="btn primary song-play-now" id="btnSongListen" type="button" aria-label="${esc(listenCtaText())}"><span class="song-listen-icon-glyph" aria-hidden="true"><svg viewBox="0 0 20 20" focusable="false"><path d="M6 4.5v11l9-5.5z"></path></svg></span><span>${esc(listenCtaText())}</span></button>`
    : "";
  const copyActionBtn = `
      ...
      <div class="card request-card">
        ...
        <form id="requestForm" class="list request-form request-form-vertical">
          ...
          <div class="request-section">
            ...
            <label class="field"><div class="muted small song-decoding-progress request-decoding-inline" id="rq_decoding">${esc(decodingProgressText(100))}</div><div class="fieldLabel">${esc(t("field.lyrics"))}</div><textarea class="textarea song-editor-text song-editor-text-main" id="rq_lyrics" wrap="off" required></textarea></label>
          </div>
    ...
    <div class="card request-card">
      ...
      <form id="requestForm" class="list request-form request-form-vertical">
        <div class="request-section">
          ...
          <details class="field ac-legacy-period" id="rq_period_wrap"><summary class="fieldLabel">${esc(t("field.period"))}</summary><select class="select" id="rq_period">${selectOptions("period", "", uiText("selectPeriod"))}</select></details>
        ...
        <div class="request-section">
          ...
          <div class="field">
            <div class="muted small song-decoding-progress request-decoding-inline" id="rq_decoding">${esc(decodingProgressText(100))}</div>
...
function songbookImportTextQualityScore(text = "", options = {}) {
  ...
  const letterCount = songbookImportCountMatches(compact, /[A-Za-zА-Яа-яЁёІіЇїЄєЎў]/g);
  const cyrillicCount = songbookImportCountMatches(compact, /[А-Яа-яЁёІіЇїЄєЎў]/g);
  const digitCount = songbookImportCountMatches(compact, /[0-9]/g);
  const weirdCount = songbookImportCountMatches(compact, /[^A-Za-zА-Яа-яЁёІіЇїЄєЎў0-9.,;:!?'"()«»„“”/\-+=_*]/g);
...
function songbookImportLineStats(line = "") {
  ...
  const letters = songbookImportCountMatches(compact, /[A-Za-zА-Яа-яЁёІіЇїЄєЎў]/g);
  const cyrillicLetters = songbookImportCountMatches(compact, /[А-Яа-яЁёІіЇїЄєЎў]/g);
  const digits = songbookImportCountMatches(compact, /[0-9]/g);
  const punctuation = songbookImportCountMatches(compact, /[.,;:!?'"()«»„“”/\-+=_*]/g);
...
function songbookImportCleanupExtractedText(text = "", options = {}) {
  ...
  for (const line of filtered) {
    ...
    if (typeof prev === "string" && prev) {
      ...
      const currentStartsLower = /^[a-zа-яёіўєії]/.test(line);
      const currentShortTail = currentStats.length <= 18 && !/^[A-ZА-ЯЁІЇЄЎ]/.test(line);
      if (prevEndsHyphen) {
        output[output.length - 1] = `${prev.replace(/[-‐‑‒–—]+$/, "")}${line}`;
        continue;
      }
...
function adminSongManageCard(song = {}, options = {}) {
  ...
  const editHref = makeHash(
    "#/admin/editor",
    {
      id: song?.id || "",
      q: filterParams.q || "",
      status: filterParams.status || "",
      recent: filterParams.recent || "",
      sort: filterParams.sort || "newest",
      page: Number(filterParams.page || 1) || 1,
    },
    ["id", "q", "status", "recent", "sort", "page"],
  );
    ...
    <div class="admin-section-stack admin-songs-stack">
      ...
      <div class="card admin-section-head">
        <div class="admin-section-head-row">
          ...
          ${canCreateSongs ? `<a class="btn ghost admin-section-head-cta" id="ac_new" href="#/admin/editor">${esc(newSongLabel)}</a>` : ``}
        </div>
      </div>
...
function adminEditorUI(song = {}, options = {}) {
  ...
  const footerActionsMarkup = (hideSongActions || (!canWriteSongs && !requestMode))
    ...
    : `
        ...
        ` : ""}
      `;
  const footerActions = requestMode && footerActionsMarkup
    ? `<div class="ac-editor-request-footer">${footerActionsMarkup}</div>`
    : footerActionsMarkup;
  const introMarkup = `
      ...
      <div class="card ac-editor-card ac-editor-page request-card song-edit-unified${requestMode ? " ac-editor-request-mode" : ""}" id="ac_editor">
        ...
        <div class="request-section ac-editor-meta-section">
          ...
          <details class="field ac-legacy-period" id="ac_period_wrap">
            ...
            <select id="ac_period" class="select">${selectOptions("period", "", uiText("selectPeriod"))}</select>
          </details>
...
function adminCountryBackgroundsUI(data) {
  ...
  const periodScopeDefaultLabel = uiLocale() === "ru" ? "Базовый фон (все эпохи)" : uiLocale() === "uk" ? "Базовий фон (усі епохи)" : uiLocale() === "et" ? "Põhitaust (kõik perioodid)" : "Default background (all eras)";
  ...
  if (route.name === "admin") {
    ...
    if (section === "editor") {
      ...
      if (isNew && !canCreateSongs && !canEditSongs) {
        return { html: `${adminTabs("songs")}<div class="card"><div class="muted">${esc(t("admin.accessDeniedDesc"))}</div></div>`, ctx: { section } };
      }
...
function bindAdminHistoricalVisuals(ctx) {
  ...
  function backgroundScopeDraft() {
    if (backgroundScope === "default") return visualProfileState.default;
    const index = scopeIndexFromValue(backgroundScope);
    return index >= 0 ? visualProfileState.variants[index] || visualProfileState.default : visualProfileState.default;
  }

  function pickInitialBackgroundScope() {
    const defaultDesktop = normalizeVisualBackground(visualProfileState.default?.desktop || {});
    const defaultMobile = normalizeVisualBackground(visualProfileState.default?.mobile || {});
    if (defaultDesktop.image_url || defaultMobile.image_url) return "default";
    const variantIndex = visualProfileState.variants.findIndex((variant) => {
      const desktop = normalizeVisualBackground(variant?.desktop || {});
      const mobile = normalizeVisualBackground(variant?.mobile || {});
      return !!(desktop.image_url || mobile.image_url);
    });
    return variantIndex >= 0 ? scopeValueForIndex(variantIndex) : "default";
  }
  ...
  function syncStickerScopeUi() {
    const label = scopeDisplayLabel(activeScope) || (uiLocale() === "ru"
      ...
      : uiLocale() === "uk"
        ...
        : uiLocale() === "et"
          ? "Vaikimisi"
          : "Default");
    const statusText = scopeHasSticker(activeScopeDraft())
      ? (uiLocale() === "ru"
        ? "Стикер есть"
        : uiLocale() === "uk"
          ? "Стікер є"
    ...
    if (scopeRefs.stickerStatus) {
      ...
      scopeRefs.stickerStatus.classList.toggle("is-set", scopeHasSticker(activeScopeDraft()));
    }
  ...
  function syncActiveScopeFromInputs() {
    const backgroundDraft = backgroundScopeDraft();
  ...
  function fillEditor(country, options = {}) {
    ...
    backgroundScope = pickInitialBackgroundScope();
    ensureActiveScope();
  ...
  scopeRefs.list?.addEventListener("click", (event) => {
    ...
    if (target.closest('[data-role="remove"]')) {
      ...
      if (backgroundScope === removedScopeValue) backgroundScope = pickInitialBackgroundScope();
      syncScopeControls();
      loadScopeIntoInputs();
      return;
    }
...
export function bind(route, ctx) {
  ...
  if (route.name === "home") {
    ...
    const topAdvancedClosedLabel = uiLocale() === "ru"
      ? "Детальный поиск"
      : uiLocale() === "uk"
        ? "Розширений пошук"
        : uiLocale() === "et"
          ? "Tapsem otsing"
          : "Advanced search";
    const topAdvancedOpenLabel = uiLocale() === "ru"
      ? "Скрыть детали"
      : uiLocale() === "uk"
        ? "Сховати деталі"
        : uiLocale() === "et"
  ...
  if (route.name === "song") {
    const song = ctx.song;
    const background = normalizeCountryBackground(ctx?.background || null, song?.country || "", { period: song?.period || "", year: song?.year || "" });
    ...
    if (songVersionTabs && lyricsMain) {
      ...
      if (compareWrap && compareToggleWrap && compareToggle && compareDiff) {
        ...
        const renderCompare = () => {
          ...
          const rowsHtml = alignedRows.length
            ? alignedRows.map((row, rowIndex) => `
              <div class="song-version-compare-row${row?.isExact ? " is-exact" : ""}" data-compare-row="${rowIndex}">
                ${renderCompareCell(leftStanzas, rightStanzas, Number.isInteger(row?.sourceIndex) ? row.sourceIndex : null, Number.isInteger(row?.targetIndex) ? row.targetIndex : null, row, rowIndex)}
                ${renderCompareCell(rightStanzas, leftStanzas, Number.isInteger(row?.targetIndex) ? row.targetIndex : null, Number.isInteger(row?.sourceIndex) ? row.sourceIndex : null, row, rowIndex)}
              </div>
            `).join("")
            : `<div class="muted small">${esc(t("common.empty"))}</div>`;
          compareDiff.innerHTML = `
            <div class="song-version-compare-head song-version-compare-head-left">
            ...
            <div class="song-version-compare-head song-version-compare-head-right">
            ...
            <div class="song-version-compare-grid">${rowsHtml}</div>
          `;
        };
  ...
  if ((route.name === "admin" && ctx.section === "editor") || route.name === "draft" || (route.name === "request" && ctx?.requestMode)) {
    ...
    const editorHistoricalBinder = bindHistoricalSelectors({
      langId: "ac_lang",
      countryId: "ac_country",
      onRefresh: () => syncPeriodVisibility("ac_country", "ac_period_wrap", "ac_period"),
      onLangChange: () => syncPeriodVisibility("ac_country", "ac_period_wrap", "ac_period"),
      onCountryChange: () => syncPeriodVisibility("ac_country", "ac_period_wrap", "ac_period"),
    });
    ...
    const setDraftBannerVisible = (visible) => {
      if (isRequestMode) editorRoot.classList.toggle("request-form-has-unsaved", !!visible);
