...
export async function onRequestPost({ env, request }){
  ...
  for(let index = 0; index < items.length; index += 1){
    ...
    await upsertFTS(env, id, it.title, it.lyrics);
    ...
    if(Array.isArray(it.versions)){
      ...
      for(const v of it.versions){
        ...
        await dbRun(env, `INSERT INTO song_versions (id,song_id,title,lang,lyrics,source,sort_order) VALUES (?,?,?,?,?,?,?)`,
          [versionId, id, normStr(v.title), normStr(v.lang), String(v.lyrics), normStr(v.source), order++]
        );
      }
    }
  }
