...
export async function onRequestPost({ env, request, params }) {
  try {
    ...
    const targetSongId = String(item.target_song_id || "").trim();
    ...
    if (requestKind === "edit_song") {
      const editPerm = await requirePermission(env, request, "songs.edit", { lang: normalizedLang });
    ...
    let songId = targetSongId || makeId("s");
    if (requestKind === "edit_song") {
      const targetSong = await dbGet(env, `SELECT id, status FROM songs WHERE id=?`, [songId]);
