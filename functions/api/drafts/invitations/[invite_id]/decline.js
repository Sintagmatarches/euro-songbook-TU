...
import { declineDraftInvitation, getDraftInvitationForUser } from "../../../../_lib/drafts.js";
...
export async function onRequestPost({ env, request, params }) {
  ...
  try {
    await declineDraftInvitation(env, { invitationId: inviteId, userId });
    const invitation = await getDraftInvitationForUser(env, { invitationId: inviteId, userId });
    return json({ ok: true, invitation });
  } catch (cause) {
    console.error("[api/drafts/invitations/:invite_id/decline] failed:", cause?.message || cause);
    return publicApiError(cause, { fallback: "Decline failed" });
  }
}
