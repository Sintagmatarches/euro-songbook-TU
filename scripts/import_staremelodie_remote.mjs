...
const REPORT_PATH = path.resolve(process.argv[3] || path.join(ROOT, "out", "staremelodie_import", "report.json"));
...
function runSqlFile(sql) {
  const file = path.join(tmpdir(), `staremelodie-import-${Date.now()}-${Math.random().toString(36).slice(2)}.sql`);
...
function songLinkSql(songId, links = []) {
  ...
  links.forEach((link, index) => {
    ...
    statements.push(
      ...
      [
        sqlValue(`sml_${songId}_${index + 1}`),
        sqlValue(songId),
...
function loadExistingSongIds() {
  const result = runSql("SELECT id FROM songs WHERE id LIKE 'sm_%' ORDER BY id;");
...
function main() {
  ...
  if (!chunkPaths.length) {
    throw new Error(`No chunk_paths in ${REPORT_PATH}`);
  }
  ...
  for (const batch of chunk(idsToDelete, 80)) {
    ...
    console.log(`Deleted stale songs ${Math.min(deletedTerms.length, idsToDelete.length)}/${idsToDelete.length}`);
  }
