...
PUB_RE = re.compile(
    ...
    r"wydawnictw|spiewnik|śpiewnik|warszaw|mosk|moscow|стрекоз|fuga|антолог|изданы|"
    r"издател|санкт[-\s]?петербург|петербург|спб\.?|гослитиздат|эксмо|композитор|"
    r"современн\w+\s+музык)",
    re.IGNORECASE,
)
...
APPEAR_RE = re.compile(r"(впервые|опублик|появил\w+|датир\w+|не\s+позднее)", re.IGNORECASE)
HIST_RE = re.compile(
    r"(восстан|революц|войн|гражданск|legion|powstani|wojna|армия|флот|битв|front)",
    re.IGNORECASE,
)
BIO_EVENT_RE = re.compile(
    r"(окончил|выпускниц|поэт\w*|композитор\w*|переводчиц\w*|дожил|факультет|университет|"
    r"консерватори|родил|умер)",
    re.IGNORECASE,
)
...
MANUSCRIPT_RE = re.compile(
    r"(из\s+песенник\w*|из\s+тетрад\w*|из\s+рукопис\w*|из\s+архив\w*|рукописн\w+\s+песенник|"
    r"manuscript|songbook|archive)",
    re.IGNORECASE,
)
RELATIVE_YEAR_PREFIX_RE = re.compile(
    r"(после|after|before|до|since|не\s+ранее|не\s+раньше|не\s+позднее|later\s+than|earlier\s+than)\s*$",
    re.IGNORECASE,
)
...
NON_SONG_TITLE_RE = re.compile(
    r"\b(вольная\s+поэзия|свадебные\s+песни|от\s+составителя|послесловие|предисловие|"
    r"введение|к\s+сборнику|учебное\s+пособие|заметки\s+к\s+истории|"
    r"песенный\s+репертуар|песенный\s+фольклор|содержание|оглавление)\b",
    re.IGNORECASE,
)
NON_SONG_LYRICS_RE = re.compile(
    ...
    r"из\s+[\"«]?википедии[\"»]?|wikipedia|см\.\s*также|see\s+also|в\s+высоком\s+разрешени|"
    r"\bплакат\b|\bplakat\b|\bposter\b|плакат\s+войны|от\s+составителя|"
    r"послесловие|предисловие|введение|содержание|оглавление)",
    re.IGNORECASE,
)
NON_SONG_META_RE = re.compile(
    r"(учебное\s+пособие|от\s+составителя|послесловие|предисловие|введение|содержание|оглавление|"
    r"кафедра|университет|институт|музеевед|песенный\s+репертуар|песенный\s+фольклор|"
    r"заметки\s+к\s+истории|выпуск\s+\d+)",
    re.IGNORECASE,
)
NON_SONG_HEADING_LINE_RE = re.compile(
    r"^\s*(от\s+составителя|послесловие|предисловие|введение|содержание|оглавление)\b",
    re.IGNORECASE,
)
NON_SONG_SECTION_LINE_RE = re.compile(r"^\s*[A-ZА-ЯЁ][^.\n]{1,100}\s+[–—-]\s+[^.\n]{2,140}$")
...
TITLE_AUTHOR_DASH_SPLIT_RE = re.compile(r"\s[-–—]\s")
AUTHOR_INITIAL_RE = re.compile(r"\b[A-ZА-ЯЁ]\.\s*[A-ZА-ЯЁ]?[a-zа-яё-]*", re.IGNORECASE)
BRACKET_CHUNK_RE = re.compile(r"\([^)]*\)|\[[^\]]*\]")
QUOTE_CHARS_RE = re.compile(r"[\"'`«»„“”‚‘’]")
TITLE_ALLOWED_CHARS_RE = re.compile(r"[^\w\s\-–—.,:;!?/№&+]", re.UNICODE)
AUTHOR_LABEL_SEGMENT_RE = re.compile(
    r"(?:музыка|music|слова|lyrics)\s*:\s*(.+?)(?=(?:\b(?:музыка|music|слова|lyrics)\s*:)|$)",
...
LYRIC_COMMENT_PREFIX_RE = re.compile(
    ...
    r"вариант|запис(?:ал|ано|ана)|опубл|published|publisher|edition|recorded|recording|"
    r"плакат|plakat|poster|wikipedia|википедии|tekst\s+pie[śs]ni|tekst\s+w\s+oryginale)\b",
    re.IGNORECASE,
)
...
HISTORICAL_YEAR_RE = re.compile(r"\b(1[0-9]{3}|20\d{2})\b")
BIRTH_YEAR_CONTEXT_RE = re.compile(r"(\bг\.\s*р\.\b|\bborn\b|\bрод\.?)", re.IGNORECASE)
...
KK_UNIQUE_CHARS = "әіңңөүұқғһӘІҢӨҮҰҚҒҺ"
...
class FilterPolicy:
    ...
    sample_seed: int = 42
    allow_movement_countries: bool = False
    allow_forced_country_by_id: bool = False
    keep_old_movement_country: bool = False
...
def is_country_ambiguous(votes: dict[str, int], lyrics_votes: dict[str, int]) -> tuple[bool, bool]:
    ...
    close_top = len(ranked) > 1 and (top_score - second_score) <= 2
...
def build_row_signals(row: SongRow, country_values: set[str], policy: FilterPolicy) -> RowSignals:
    ...
    kk_unique_chars = count_chars(lyrics_song, KK_UNIQUE_CHARS)
    uz_unique_chars = count_chars(lyrics_song, "ҳҲ")
    ...
    uz_word_marker_count = count_marker_words(tokens, UZ_WORD_MARKERS)
    ...
    return RowSignals(
        ...
        kk_unique_chars=kk_unique_chars,
        uz_unique_chars=uz_unique_chars,
        uk_word_marker_count=uk_word_marker_count,
        be_word_marker_count=be_word_marker_count,
        uz_word_marker_count=uz_word_marker_count,
...
def has_strong_uz_signal(signals: RowSignals) -> bool:
    if signals.uz_unique_chars >= 2:
        return True
    if signals.uz_unique_chars >= 1 and signals.uz_word_marker_count >= 1:
        return True
    if signals.uz_word_marker_count >= 3 and signals.kk_unique_chars <= max(1, signals.uz_unique_chars):
        return True
    if (
        signals.uz_word_marker_count >= 2
        and signals.cyr_count >= max(18, int(signals.lat_count * 1.05))
        and signals.kk_unique_chars <= max(2, signals.uz_unique_chars * 2)
    ):
        return True
    return False
...
def has_strong_cyr_lang_signal(signals: RowSignals) -> bool:
    return has_strong_uk_signal(signals) or has_strong_be_signal(signals) or has_strong_uz_signal(signals)
...
def force_cyr_lang_aggressive(
    ...
    uz_strong = has_strong_uz_signal(signals)
    ...
    if not cyr_dominant and not (uk_strong or be_strong or uz_strong):
        return pred_lang, reason, False
    if uz_strong and signals.uz_marker_count >= max(signals.uk_marker_count, signals.be_marker_count):
        return "uz", "uz_marker", True
...
def classify_language(
    ...
    base_lang = old_lang
    # For strong latin-script lyrics, avoid fallback-to-old-cyr bias in the classifier.
    if lat_dominant and old_lang in cyr_langs:
        base_lang = "en"
    core = lang_mod.SongRow(row.id, signals.title_norm, signals.subtitle_norm, base_lang, signals.lyrics_song, "", "")
    pred, conf, reason = lang_mod.classify_song(core)

    # For short/mixed latin texts, include cleaned metadata to avoid cyr comment bias.
    if (
        lat_dominant
        and len(signals.lyrics_song) < 1200
        and (pred in {"lb", "fo", "ga", "mt", "cy", "sq", "eu", "gl", "ca"} or float(conf) < 0.92)
    ):
        full = lang_mod.SongRow(
            row.id,
            signals.title_norm,
            signals.subtitle_norm,
            base_lang,
            signals.lyrics_song,
            signals.source_clean[:2400],
            signals.notes_clean[:2400],
        )
        p2, c2, r2 = lang_mod.classify_song(full)
        if p2 in lang_values and float(c2) >= float(conf):
            pred, conf, reason = p2, float(c2), str(r2)

    # Latin original + Polish context frequently appears with brief RU annotations.
    if lat_dominant and pred in {"lb", "fo", "ga", "mt", "cy", "sq", "eu", "gl", "ca", "en"}:
        scope = signals.scope_short.lower()
        if re.search(r"(polsk|polonia|warsz|oryginale\s+łaciń|oryginale\s+lacins|tekst\s+pie[śs]ni)", scope):
            pred, conf, reason = "pl", max(float(conf), 0.985), "latin_polish_context"
    ...
    # Yiddish transliteration guard: latin-dominant text with stable YI markers must not stay in cyr languages.
    if lat_dominant and signals.cyr_count <= 12 and signals.yi_marker_count >= 3:
        pred, conf, reason = "yi", max(float(conf), 0.99), "yi_marker"
    elif lat_dominant and signals.cyr_count <= 12 and pred == "lb" and signals.yi_marker_count >= 2:
        pred, conf, reason = "yi", max(float(conf), 0.98), "yi_marker_from_lb"
...
def should_apply_language_change(old_lang: str, new_lang: str, confidence: float, reason: str, lang_mod) -> bool:
    if old_lang == new_lang:
        return False
    if new_lang == "yi" and ("yi_marker" in reason):
        return True
...
def extract_year_candidates(text: str, origin: str) -> list[YearCandidate]:
    ...
    for raw_line in src.splitlines() or [src]:
        line = raw_line.strip()
        if not line:
            continue
        line_has_pub = bool(PUB_RE.search(line.lower()))
        safe_line = re.sub(r"\b(спб|г|р)\.", lambda m: m.group(1) + "<dot>", line, flags=re.IGNORECASE)
        safe_line = re.sub(r"\b([A-ZА-ЯЁ])\.", r"\1<dot>", safe_line)
        segments = [segment.replace("<dot>", ".").strip() for segment in re.split(r"(?<=[.!?;])\s+", safe_line) if segment.strip()] or [line]
        for idx, segment in enumerate(segments):
            lower = segment.lower()
            seg_pub = bool(PUB_RE.search(lower))
            seg_manuscript = bool(MANUSCRIPT_RE.search(lower))
            seg_record = bool(RECORD_RE.search(lower))
            seg_bio = bool(BIO_EVENT_RE.search(lower))
            seg_derivative = bool(DERIVATIVE_RE.search(lower))
            seg_tokens = tokenize_words(segment)
            seg_ranges = list(LIFESPAN_RE.finditer(segment))
            for m in YEAR_RE.finditer(segment):
                y = int(m.group(1))
                if not (YEAR_MIN <= y <= YEAR_MAX):
                    continue
                a, b = max(0, m.start() - 48), min(len(segment), m.end() + 48)
                ctx = lower[a:b]
                birth_ctx = lower[max(0, m.start() - 8) : min(len(segment), m.end() + 16)]
                before = lower[max(0, m.start() - 24) : m.start()]
                after = lower[m.end() : min(len(segment), m.end() + 16)]
                score = base
                tags = [origin]
                if origin in {"source", "notes"} and m.start() <= 4:
                    score += 8
                    tags.append("lead")
                if COMPOSE_RE.search(ctx):
                    score += 8
                    tags.append("compose")
                if APPEAR_RE.search(ctx):
                    score += 4
                    tags.append("appear")
                if HIST_RE.search(ctx):
                    score += 3
                    tags.append("historic")
                if seg_bio or BIO_EVENT_RE.search(ctx):
                    score -= 6
                    tags.append("bio")
                if seg_derivative or DERIVATIVE_RE.search(ctx):
                    score -= 10
                    tags.append("derivative")
                if seg_manuscript and y < 1990:
                    score += 5
                ...
                if re.match(r"\s*[-–—]\s*(?:\d{1,2}|[хе])", after):
                    score -= 6
                    tags.append("decade_span")
                if seg_pub or PUB_RE.search(ctx):
                    score -= 8
                    tags.append("pub")
                    if origin in {"source", "notes"} and y >= 1990:
                        score -= 6
                        tags.append("pub_modern")
                    if m.start() >= max(18, int(len(segment) * 0.55)):
                        score -= 3
                        tags.append("pub_tail")
                elif (
                    line_has_pub
                    and idx == len(segments) - 1
                    and not seg_record
                    and len(seg_tokens) <= 6
                    and y >= 1900
                ):
                    score -= 8
                    tags.append("pub")
                    if origin in {"source", "notes"} and y >= 1990:
                        ...
                        tags.append("pub_modern")
                    tags.append("pub_tail")
                if BIRTH_RE.search(ctx) or re.search(r"г\.\s*р\.", birth_ctx):
                    score -= 12
                    tags.append("lifespan")
                else:
                    for rng in seg_ranges:
                        start_year = int(rng.group(1))
                        end_year = int(rng.group(2))
                        if y not in {start_year, end_year}:
                            continue
                        if seg_pub:
                            score -= 6
                            tags.append("range_pub")
                        else:
                            score -= 12
                            tags.append("lifespan")
                        break
                if (
                    origin in {"source", "notes"}
                    and y >= 2010
                    and not {"compose", "historic", "manuscript", "record"} & set(tags)
                ):
                    score -= 14
                    tags.append("modern_meta")
                out.append(YearCandidate(y, score, "+".join(tags), origin))
    return out


def year_candidate_tags(candidate: YearCandidate) -> set[str]:
    return {part for part in candidate.reason.split("+") if part}


def is_publication_only_candidate(candidate: YearCandidate) -> bool:
    tags = year_candidate_tags(candidate)
    if candidate.origin not in {"source", "notes"}:
        return False
    if "pub" not in tags:
        return False
    return not ({"compose", "historic", "manuscript", "record", "lead"} & tags)


def publication_only_old_year(old_year: int | None, candidates: list[YearCandidate]) -> bool:
    if old_year is None or old_year < 1990:
        return False
    matching = [cand for cand in candidates if cand.origin in {"source", "notes"} and cand.year == old_year]
    if not matching:
        return False
    return all(is_publication_only_candidate(cand) for cand in matching)


def year_candidate_rank(candidate: YearCandidate) -> tuple[int, int]:
    year_bias = -candidate.year if is_publication_only_candidate(candidate) else candidate.year
    return candidate.score, year_bias
...
SCRIPT_CYR_LANGS = {"ru", "uk", "be", "bg", "mk", "sr", "kk", "uz"}
...
def looks_like_author_prefix(text: str) -> bool:
    s = normalize_inline(text)
    if not s or len(s) > 140:
        return False
    if AUTHOR_INITIAL_RE.search(s):
        return True
    # Fallback: short list with separators and titlecase words.
    parts = [p.strip() for p in re.split(r"[;,]", s) if p.strip()]
    if len(parts) < 2:
        return False
    token_count = sum(len(tokenize_words(part)) for part in parts)
    if token_count > 16:
        return False
    upperish = sum(1 for ch in s if ch.isalpha() and ch.isupper())
    lowerish = sum(1 for ch in s if ch.isalpha() and ch.islower())
    return upperish >= max(2, int(lowerish * 0.3))


def normalize_author_chunk(text: str) -> str:
    s = normalize_inline(text)
    s = BRACKET_CHUNK_RE.sub(" ", s)
    s = QUOTE_CHARS_RE.sub("", s)
    s = TITLE_ALLOWED_CHARS_RE.sub(" ", s)
    s = re.sub(r"\s{2,}", " ", s).strip(" ,;:-.")
    return s


def sanitize_title_text(title: str) -> str:
    s = normalize_inline(title)
    if not s:
        return ""
    s = BRACKET_CHUNK_RE.sub(" ", s)
    s = s.replace("<", " ").replace(">", " ")
...
ROMAN_NUMERAL_TOKEN_RE = re.compile(r"^(?=[MDCLXVI]+$)M{0,4}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})$", re.IGNORECASE)
...
def normalize_title_case(title: str) -> str:
    ...
    if not (lower == 0 or (upper / max(1, len(letters))) >= 0.85):
        return s
    ...
    for token in tokens:
        if not token or token.isspace():
            out.append(token)
            continue
        plain = re.sub(r"[^A-Za-zА-Яа-яЁёІіЇїЄєҐґ]", "", token)
        if plain and ROMAN_NUMERAL_TOKEN_RE.match(plain):
            out.append(token.upper())
            continue
        if len(plain) == 1 and plain.isupper():
            out.append(token)
            continue
        lowered = token.lower()
        out.append(lowered)
    ...
    if not joined:
        return s
    # Capitalize first alpha letter only.
    chars = list(joined)
...
def extract_authors_from_title(title: str) -> tuple[str, str]:
    src = normalize_inline(title)
    if not src:
        return "", ""
    parts = TITLE_AUTHOR_DASH_SPLIT_RE.split(src, maxsplit=1)
    if len(parts) != 2:
        return src, ""
    prefix, rest = parts[0].strip(), parts[1].strip()
    if not looks_like_author_prefix(prefix):
        return src, ""
    return rest, normalize_author_chunk(prefix)


def canonical_author_subtitle(subtitle: str) -> str:
    s = normalize_inline(subtitle)
    if not s:
        return ""

    segments = [normalize_author_chunk(m) for m in AUTHOR_LABEL_SEGMENT_RE.findall(s)]
    authors: list[str] = []
    seen_authors: set[str] = set()
    for seg in segments:
        if not seg:
            continue
        parts = [normalize_author_chunk(p) for p in re.split(r"[;,]|(?:\s+и\s+)|(?:\s+and\s+)|&", seg, flags=re.IGNORECASE)]
        for p in parts:
            if not p:
                continue
            low = p.lower()
            if low not in seen_authors:
                authors.append(p)
                seen_authors.add(low)
    if authors:
        return f"Музыка и слова: {', '.join(authors)}"

    if re.match(r"^\s*(музыка|слова|music|lyrics)\s*:", s, flags=re.IGNORECASE):
        payload = re.sub(r"^\s*(музыка|слова|music|lyrics)\s*:\s*", "", s, flags=re.IGNORECASE).strip()
        payload = normalize_author_chunk(payload)
        if payload:
            return f"Музыка и слова: {payload}"
    return s
...
def repair_title_from_lyrics(title: str, subtitle: str, lyrics: str, notes: str) -> tuple[str, str, str, str, bool]:
    clean_title = normalize_inline(title)
    clean_subtitle = normalize_inline(subtitle)
    clean_lyrics = normalize_newlines(lyrics).strip()
    clean_notes = normalize_newlines(notes).strip()
    repaired = False

    extracted_title, author_chunk = extract_authors_from_title(clean_title)
    clean_title = extracted_title
    if author_chunk:
        author_label = f"Музыка и слова: {author_chunk}"
        if not clean_subtitle:
            clean_subtitle = author_label
        elif author_chunk.lower() not in clean_subtitle.lower():
            clean_subtitle = f"{author_label}. {clean_subtitle}"
    clean_subtitle = canonical_author_subtitle(clean_subtitle)
    ...
    clean_title = sanitize_title_text(clean_title)
    clean_title = normalize_title_case(clean_title)
    if not clean_title:
        clean_title = normalize_title_case(sanitize_title_text(title))
    if not clean_subtitle:
        # Keep subtitle as-is only if we didn't extract authors.
        clean_subtitle = canonical_author_subtitle(subtitle)

    return clean_title, clean_subtitle, clean_lyrics, clean_notes, repaired
...
def validate_row_update(
    *,
    ...
    be_strong = has_strong_be_signal(signals)
    uz_strong = has_strong_uz_signal(signals)
    if uz_strong and out_lang != "uz":
        validation_flags["lang_scope_conflict"] += 1
    elif uk_strong and not be_strong and out_lang != "uk":
        validation_flags["lang_scope_conflict"] += 1
...
def is_probably_non_song(title: str, lyrics: str, source: str = "", notes: str = "") -> bool:
    t = norm_text(title)
    l = norm_text(lyrics)
    meta = "\n".join([norm_text(source), norm_text(notes)])
    if NON_SONG_TITLE_RE.search(t):
        return True
    if NON_SONG_LYRICS_RE.search(l):
        return True
    lines = [line.strip() for line in normalize_newlines(l).splitlines() if line.strip()]
    heading_hits = sum(1 for line in lines[:10] if NON_SONG_HEADING_LINE_RE.search(line) or NON_SONG_SECTION_LINE_RE.match(line))
    if heading_hits >= 3:
        return True
    if NON_SONG_LYRICS_RE.search(meta) and (heading_hits >= 1 or (len(tokenize_words(l)) <= 4 and len(lines) < 3)):
        return True
    if heading_hits >= 2 and (NON_SONG_META_RE.search(t) or NON_SONG_META_RE.search(meta)):
        return True
    if NON_SONG_META_RE.search(t) and NON_SONG_META_RE.search(meta):
        return True
    return False
...
def normalized_title_for_dedupe(title: str) -> str:
    t = normalize_inline(title)
    t, _authors = extract_authors_from_title(t)
    t = sanitize_title_text(t)
    t = normalize_title_case(t)
    return normalize_for_dedupe(t)


def duplicate_drop_ids(rows: list[SongRow]) -> tuple[dict[str, str], Counter[str], Counter[str]]:
    """
    Detect duplicates and return:
    1) map duplicate_row_id -> keeper_row_id
    2) extra-row counts by duplicate reason
    3) group counts by duplicate reason
    """

    def _rank(r: SongRow) -> tuple[int, int, str, str]:
        # Keep earliest non-tg3 published row when possible.
    ...
    by_exact_title_lyrics: dict[tuple[str, str, str], list[SongRow]] = defaultdict(list)
    by_lyrics: dict[tuple[str, str], list[SongRow]] = defaultdict(list)
    by_title_year_lang: dict[tuple[str, str, str], list[SongRow]] = defaultdict(list)

    for row in rows:
        lang = row.lang or "ru"
        key_lyrics = normalize_for_dedupe(row.lyrics)
        key_title = normalized_title_for_dedupe(row.title)
        key_year = normalize_inline(row.year)
        if key_title and key_lyrics:
            by_exact_title_lyrics[(lang, key_title, key_lyrics)].append(row)
        # Fuzzy fallback for same-lyrics clusters; lowered threshold catches short exact copies.
        if len(key_lyrics) >= 80:
            by_lyrics[(lang, key_lyrics)].append(row)
        if key_title and key_year and len(key_title) >= 6 and len(key_lyrics) >= 80:
            by_title_year_lang[(lang, key_title, key_year)].append(row)

    drops: dict[str, str] = {}
    duplicate_reason_counts: Counter[str] = Counter()
    duplicate_group_counts: Counter[str] = Counter()

    for group in by_exact_title_lyrics.values():
        if len(group) <= 1:
            continue
        keeper = sorted(group, key=_rank)[0]
        duplicate_group_counts["exact_title_lyrics"] += 1
        for row in group:
            if row.id == keeper.id:
                continue
            if row.id in drops:
            ...
            duplicate_reason_counts["exact_title_lyrics"] += 1

    for (_lang, _lyrics_key), group in by_lyrics.items():
        if len(group) <= 1:
            continue
        keeper = sorted(group, key=_rank)[0]
        keeper_title = normalized_title_for_dedupe(keeper.title)
        grouped = False

        for row in group:
            if row.id == keeper.id or row.id in drops:
                continue
            title_norm = normalized_title_for_dedupe(row.title)
            ...
            if title_norm == keeper_title or title_norm in keeper_title or keeper_title in title_norm or similar:
                drops[row.id] = keeper.id
                duplicate_reason_counts["lyrics_title_fuzzy"] += 1
                grouped = True

        if grouped:
            duplicate_group_counts["lyrics_title_fuzzy"] += 1

    for (_lang, _title_key, _year_key), group in by_title_year_lang.items():
        if len(group) <= 1:
            continue
        keeper = sorted(group, key=_rank)[0]
        grouped = False
        keeper_lyrics = normalize_for_dedupe(keeper.lyrics)
        for row in group:
            if row.id == keeper.id or row.id in drops:
                continue
            row_lyrics = normalize_for_dedupe(row.lyrics)
            if not row_lyrics or not keeper_lyrics:
                continue
            max_len = max(len(row_lyrics), len(keeper_lyrics))
            len_gap = abs(len(row_lyrics) - len(keeper_lyrics))
            if max_len == 0 or (len_gap / max_len) > 0.35:
                continue
            similar = difflib.SequenceMatcher(a=row_lyrics, b=keeper_lyrics).ratio() >= 0.965
            if row_lyrics == keeper_lyrics or row_lyrics in keeper_lyrics or keeper_lyrics in row_lyrics or similar:
                drops[row.id] = keeper.id
                duplicate_reason_counts["title_year_lyrics_strict"] += 1
                grouped = True
        if grouped:
            duplicate_group_counts["title_year_lyrics_strict"] += 1

    return drops, duplicate_reason_counts, duplicate_group_counts
...
def infer_song_year(row: SongRow, signals: RowSignals | None = None) -> tuple[int | None, str]:
    ...
    best = max(cands, key=year_candidate_rank)
    evidence = [cand for cand in cands if cand.origin != "existing" and not is_publication_only_candidate(cand)]
    if is_publication_only_candidate(best) and evidence:
        alt = max(evidence, key=year_candidate_rank)
        if alt.score >= (best.score - 4):
            best = alt
    if old is not None:
        old_evidence = [cand for cand in evidence if cand.year == old]
        if old_evidence:
            old_best = max(old_evidence, key=year_candidate_rank)
            if old_best.score >= (best.score - 4):
                best = old_best
    meta_years = [c.year for c in cands if c.origin in {"source", "notes"} and c.score >= 2]
    if (
        old is None
        and best.origin in {"source", "notes"}
        and len(set(meta_years)) >= 3
        and (max(meta_years) - min(meta_years)) >= 40
        and "compose" not in best.reason
    ):
        return None, "ambiguous_meta_year"
    if publication_only_old_year(old, cands):
        older_evidence = [cand for cand in evidence if cand.year <= old - 20 and cand.score >= 1]
        if older_evidence:
            best = max(older_evidence, key=year_candidate_rank)
        else:
            return None, "drop_publication_year_only"
    if old is not None:
        best_tags = year_candidate_tags(best)
        if best.year >= 2010 and old <= 2005 and best.origin in {"source", "notes"}:
            return old, "keep_old_modern_meta"
        if best.origin == "notes" and best_tags.issubset({"notes", "lead"}) and abs(best.year - old) > 35:
            return old, "keep_old_far_meta"
        if (
            best.origin in {"source", "notes"}
            and best.score < 12
            and abs(best.year - old) > 35
            and not ({"lead", "manuscript", "compose", "historic", "record"} & best_tags)
        ):
            return old, "keep_old_far_meta"
    if best.score < 1:
        if publication_only_old_year(old, cands):
...
def infer_country_context_year(row: SongRow, signals: RowSignals, current_year: int | None) -> int | None:
    if current_year is not None:
        return current_year

    scored: list[tuple[int, int]] = []
    sources = [
        ("title", normalize_newlines(row.title), 9),
        ("subtitle", normalize_newlines(row.subtitle), 7),
        ("source", signals.source_clean, 6),
        ("notes", signals.notes_clean, 4),
        ("lyrics", signals.lyrics_song[:1200], 2),
    ]

    for _origin, text, base in sources:
        if not text:
            continue
        lower = text.lower()
        for m in HISTORICAL_YEAR_RE.finditer(text):
            ...
            if year > YEAR_MAX:
                continue
            a, b = max(0, m.start() - 42), min(len(text), m.end() + 42)
            ctx = lower[a:b]
            score = base
            if COMPOSE_RE.search(ctx):
                score += 4
            ...
            if RECORD_RE.search(ctx):
                score -= 2
            if BIRTH_YEAR_CONTEXT_RE.search(ctx):
                score -= 8
    ...
    if not scored:
        return None
    score, year = max(scored, key=lambda item: (item[0], -item[1]))
    if score < 5:
        return None
    return year
...
def canonical_country_value(raw_country: str, aliases: dict[str, str], country_values: set[str]) -> str:
    folded = fold_alias(raw_country)
    mapped = aliases.get(folded, "")
    if mapped in country_values:
        return mapped
    raw_norm = fold(raw_country)
    if raw_norm in country_values:
        return raw_norm
    return ""
...
def timeline_poland(year: int | None, country_values: set[str]) -> str:
    ...
    if year <= 1917:
        return pick_existing(country_values, "kingdom_of_poland_within_russian_empire_1900_1915", "poland_1989")
    if year <= 1951:
        return pick_existing(country_values, "second_polish_republic_1918_1939", "poland_1989")
    if year <= 1989:
        return pick_existing(country_values, "polish_peoples_republic_1952_1989", "poland_1989")
    return pick_existing(country_values, "poland_1989")
...
def timeline_country_for_lang(
    lang: str,
    year: int | None,
    scope: str,
    country_values: set[str],
    allow_movement_countries: bool,
) -> str:
    s = scope.lower()

    if allow_movement_countries:
    ...
    if lang == "uz":
        if year is None:
            return pick_existing(country_values, "uzbekistan_1991", "uzbek_ssr_1924_1991")
        if year <= 1991:
            return pick_existing(country_values, "uzbek_ssr_1924_1991", "uzbekistan_1991")
        return pick_existing(country_values, "uzbekistan_1991")
...
def classify_country(
    ...
    old_country = canonical_country_value(row.country, aliases, country_values)

    if policy.allow_forced_country_by_id:
    ...
    if policy.keep_old_movement_country and old_country in MOVEMENT_COUNTRIES and new_country not in MOVEMENT_COUNTRIES:
...
def load_songs(path: Path) -> list[SongRow]:
    ...
    rows = conn.execute(
        ...
        SELECT
          id,
          replace(coalesce(title,''), char(13), ''),
          replace(coalesce(subtitle,''), char(13), ''),
          lower(trim(coalesce(lang,''))),
          lower(trim(coalesce(country,''))),
          coalesce(year,''),
          replace(coalesce(lyrics,''), char(13), ''),
          replace(coalesce(source,''), char(13), ''),
          replace(coalesce(notes,''), char(13), ''),
...
def build_updates(
    ...
    for row in rows:
        ...
        clean_title, clean_subtitle, clean_lyrics, clean_notes, title_repaired = repair_title_from_lyrics(
            clean_title,
            clean_subtitle,
            clean_lyrics,
            clean_notes,
        ...
        old_country = canonical_country_value(row.country, aliases, country_values)
        old_country_raw = fold(row.country)
        ...
        status_reason = ""
        if is_probably_non_song(clean_title, clean_lyrics, clean_source, clean_notes):
            new_status = "draft"
        ...
        if new_country != old_country or (old_country and old_country_raw != old_country):
            ...
            country_pairs[(old_pair_value, new_country)] += 1
    ...
    report = {
        ...
        "duplicate_reason_counts": [{"reason": r, "count": c} for r, c in duplicate_reason_counts.most_common()],
        ...
        "policy": {
            ...
            "allow_movement_countries": policy.allow_movement_countries,
            "allow_forced_country_by_id": policy.allow_forced_country_by_id,
            "keep_old_movement_country": policy.keep_old_movement_country,
        },
...
def main() -> None:
    ...
    parser.add_argument("--allow-movement-countries", action="store_true")
    parser.add_argument("--allow-forced-country-by-id", action="store_true")
    parser.add_argument("--keep-old-movement-country", action="store_true")
    ...
    policy = FilterPolicy(
        ...
        allow_movement_countries=bool(args.allow_movement_countries),
        allow_forced_country_by_id=bool(args.allow_forced_country_by_id),
        keep_old_movement_country=bool(args.keep_old_movement_country),
    )
