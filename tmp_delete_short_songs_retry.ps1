...
for ($offset = 0; $offset -lt $ids.Count; $offset += 40) {
  $last = [Math]::Min($offset + 39, $ids.Count - 1)
  ...
  $out = npx wrangler d1 execute euro-songbook-db --remote --command $sql 2>&1 | Out-String
  if ($LASTEXITCODE -ne 0 -or $out -match 'The command line is too long') {
