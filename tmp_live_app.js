...
function getRegisterErrorMessage(error) {
  ...
  if (message.includes("nickname")) {
    if (state.locale === "ru") return "??N??????µN?N?N??µ ??????: N??¶?µ ?·?°??N?N? ???»?? N???N????°N? ???µ??????N?N?N?????.";
    if (state.locale === "uk") return "???µN??µ??N?N?N??µ ??N???: N??¶?µ ?·?°????N?N????? ?°?±?? N???N????°N? ???µ??N???N???????.";
    if (state.locale === "et") return "Kontrolli kasutajanime: juba hAµivatud vAµi vorming vale.";
    return "Check nickname: already taken or invalid format.";
  }
  if (message.includes("passwords do not match")) {
    if (state.locale === "ru") return "???°N????»?? ???µ N????????°???°N?N?.";
    if (state.locale === "uk") return "???°N????»N? ???µ ?·?±N????°N?N?N?N?N?.";
    if (state.locale === "et") return "Paroolid ei A?hti.";
...
function authNicknameLabel() {
  if (state.locale === "ru") return "?????? ???»?? email (???»N? ??N??????°)";
  if (state.locale === "uk") return "??N??? ?°?±?? email (???»N? ??N?????N?)";
  if (state.locale === "et") return "Kasutajanimi vAµi email (sisselogimiseks)";
  return "Nickname or email (for login)";
}

function authNicknameRegisterLabel() {
  if (state.locale === "ru") return "?????? (???»N? N??µ????N?N?N??°N?????)";
  if (state.locale === "uk") return "??N??? (???»N? N??µN?N?N?N??°N?N?N?)";
  if (state.locale === "et") return "Kasutajanimi (registreerimiseks)";
  return "Nickname (for registration)";
}

function authEmailRegisterLabel() {
  if (state.locale === "ru") return "?­?». ????N?N??°";
  if (state.locale === "uk") return "???». ????N?N??°";
  if (state.locale === "et") return "Email";
  return "Email";
}

function authPasswordConfirmLabel() {
  if (state.locale === "ru") return "??????N???N???N??µ ???°N????»N?";
  if (state.locale === "uk") return "??????N???N?N?N?N? ???°N????»N?";
  if (state.locale === "et") return "Korda parooli";
  return "Repeat password";
}

function authNickAndPassRequiredMessage() {
  if (state.locale === "ru") return "?????µ????N??µ ??????/email ?? ???°N????»N?.";
  if (state.locale === "uk") return "?????µ??N?N?N? ??N???/email N? ???°N????»N?.";
...
function authRegisterFieldsRequiredMessage() {
  if (state.locale === "ru") return "???»N? N??µ????N?N?N??°N????? ??N??¶??N? ??????, email ?? ???°N????»N?.";
  if (state.locale === "uk") return "???»N? N??µN?N?N?N??°N?N?N? ????N?N?N??±??N? ??N???, email N? ???°N????»N?.";
...
function topSearchText() {
  if (state.locale === "ru") return { action: "?˜N????°N?N?", placeholder: "?˜N????°N?N?a?¦" };
  if (state.locale === "uk") return { action: "??N????°N???", placeholder: "??N????°N???a?¦" };
  if (state.locale === "et") return { action: "Otsi", placeholder: "Otsia?¦" };
  return { action: "Search", placeholder: "Searcha?¦" };
}

function installButtonText() {
  if (state.locale === "ru") return "??N?N??°????????N?N? ??N????»???¶?µ?????µ";
  if (state.locale === "uk") return "??N?N??°????????N??? ?·?°N?N???N?N???????";
...
function addSongNavText() {
  if (state.locale === "ru") return "?????±?°????N?N? ???µN???N?";
  if (state.locale === "uk") return "???????°N??? ??N?N???N?";
  if (state.locale === "et") return "Lisa laul";
  return "Add song";
}

function brandSubtitleText() {
  if (state.locale === "ru") return "????N??????µ??N??????? ??N?N???N???N??µN??????? ???µN??µ????????";
  if (state.locale === "uk") return "????N??????µ??N?N??????? N?N?N???N???N??????? ??N?N??µ????????";
...
function appBootLoadingText() {
  if (state.locale === "ru") return "???°??N?N??·???°...";
  if (state.locale === "uk") return "???°???°??N??°?¶?µ????N?...";
