    ...
    <div class="ac-line-popover-card ${draftConfidenceBand(activeConfidence)}" data-line-id="${esc(normalizedLine.id)}" style="${draftConfidenceStyleValue(activeConfidence)}">
      ...
      <div class="draft-variants-panel">
        ${variants.map((variant, variantIndex) => {
          ...
          const wordEditorMarkup = isActive
            ? draftWordConfidenceEditorUI(
              normalizedLine.id,
              variantId,
              variantText,
          ...
          <div class="draft-variant-row ${draftConfidenceBand(variantConfidence)} ${isActive ? "is-active" : ""}" style="${draftConfidenceStyleValue(variantConfidence)}">
            <div class="draft-variant-main">
              ...
              <div class="draft-variant-footer">
                <label class="draft-variant-confidence-field">
                  <span class="draft-confidence-level-label">${esc(draftUiText("confidenceLevel"))}</span>
                  <span class="draft-variant-confidence-wrap">
                    <select class="select draft-variant-confidence-select" data-line-id="${esc(normalizedLine.id)}" data-variant-id="${esc(variantId)}" aria-label="${esc(draftUiText("confidenceLevel"))}">
                      ${draftConfidenceSelectOptions(variantConfidence)}
                    </select>
                  </span>
                </label>
                <div class="draft-variant-controls">
                  ...
                  <button class="btn ghost draft-variant-remove" type="button" data-line-id="${esc(normalizedLine.id)}" data-variant-id="${esc(variantId)}" ${canRemoveVariant ? "" : "disabled"} aria-label="${esc(draftUiText("removeVariantTitle"))}" title="${esc(draftUiText("removeVariantTitle"))}">${esc(draftUiText("removeVariant"))}</button>
                </div>
              </div>
              ${wordEditorMarkup}
            </div>
...
function songConfidenceWordStyleValue(confidence) {
  ...
  const ratio = Math.max(0, Math.min(1, safeConfidence / 100));
  const red = Math.round(232 - (122 * ratio));
  const green = Math.round(74 + (118 * ratio));
  const blue = Math.round(92 + (32 * ratio));
  const fillAlpha = 0.12 + ((1 - ratio) * 0.1);
  const borderAlpha = 0.24 + ((1 - ratio) * 0.12);
  return [
...
function renderTextWithConfidenceWords(text, segments = []) {
  const source = String(text || "");
  ...
  for (const segment of normalizedSegments) {
    const start = Math.max(cursor, Number(segment?.start || 0));
    const end = Math.max(start, Number(segment?.end || start));
    if (start > cursor) {
      out += renderTextWithUnknownMarkers(source.slice(cursor, start));
    }
    if (end > start) {
      const confidence = draftClampConfidence(segment?.confidence, 100);
      if (confidence >= 100) {
        out += renderTextWithUnknownMarkers(source.slice(start, end));
        cursor = end;
        continue;
      }
      out += `<span class="song-confidence-word" data-confidence-label="${esc(draftConfidencePercent(confidence, 100))}" style="${songConfidenceWordStyleValue(confidence)}">${renderTextWithUnknownMarkers(source.slice(start, end))}</span>`;
      cursor = end;
      continue;
    }
...
function renderSongLyricVariantsPanel(meta = null) {
  ...
  const variantsMarkup = variants.length
    ? variants.map((variant) => `
      <div class="song-line-variant-row ${variant?.is_active ? "is-active" : ""}">
        <div class="song-line-variant-text">${String(variant?.text || "").trim() ? renderTextWithUnknownMarkers(String(variant?.text || "")) : "&nbsp;"}</div>
        <div class="song-line-variant-meta">
          <span class="badge">${esc(variant?.is_active ? draftUiText("activeVariant") : draftVariantTypeLabel(variant?.variant_type || "manual"))}</span>
          <span class="badge">${esc(draftConfidenceLabel(variant?.confidence || 100))}</span>
        </div>
      </div>
    `).join("")
  ...
  return `
    <div class="song-line-variants-panel">
      <div class="song-line-variants-head">
        <span class="badge">${esc(draftUiText("variantsList"))}</span>
        <span class="badge">${esc(String(totalCount))}</span>
      </div>
      <div class="song-line-variants-list">${variantsMarkup}</div>
    </div>
  `;
}
...
function renderLyricLineWithConfidence(text, meta = null) {
  ...
  const safeSegments = draftNormalizeSegments(safeMeta?.confidence_segments, content);
  const lineAccentConfidence = songLineAccentConfidence(safeSegments);
  ...
  const classes = [
    ...
    hasVariantMenu ? "has-tools" : "",
    hasVariantMenu ? "has-variants" : "",
  ].filter(Boolean).join(" ");
  const styleAttr = hasWordConfidence ? ` style="${songConfidenceWordStyleValue(lineAccentConfidence)}"` : "";
  const controlsMarkup = hasVariantMenu
    ? `
      <div class="song-lyric-line-tools">
        <details class="song-line-variants-details">
          <summary class="song-line-variants-toggle" aria-label="${esc(draftUiText("variantsToggle"))}" title="${esc(draftUiText("variantsToggle"))}">
            <span aria-hidden="true">${variantsCount}</span>
          </summary>
          ${variantsPanel}
        </details>
      </div>
    `
    : "";
  return `<div class="${classes}"${styleAttr}><div class="song-lyric-line-text">${content ? renderTextWithConfidenceWords(content, safeSegments) : "&nbsp;"}</div>${controlsMarkup}</div>`;
}
  ...
  if (route.name === "request") {
    ...
    const initRequestLineAssistant = () => {
      ...
      const positionPopover = () => {
        ...
        const popoverWidth = Math.max(180, Math.min(Math.round(textRect.width), availableWidth));
