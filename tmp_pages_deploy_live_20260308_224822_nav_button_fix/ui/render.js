    ...
    <section class="yt-main home-main">
      <div class="yt-detail-trigger" id="yt_detailToggleWrap">
        <div class="yt-detail-trigger-actions">
          ...
          ><span class="home-cta-button__label">${esc(showAdvanced ? hideAdvancedLabel : advancedLabel)}</span><span class="home-cta-button__chevron" aria-hidden="true">›</span></button>
          <a class="btn ghost home-cta-button ${recent ? "is-active" : ""}" id="yt_openRecent" href="${esc(recentEntryHref)}"><span class="home-cta-button__label">${esc(homeRecentSongsLabel())}</span><span class="home-cta-button__chevron" aria-hidden="true">›</span></a>
        </div>
      </div>
...
export function bind(route, ctx) {
  ...
  if (route.name === "home") {
    ...
    qs("yt_toggleAdvanced")?.addEventListener("click", () => {
      ...
      if (toggleButton) {
        const labelNode = toggleButton.querySelector(".home-cta-button__label");
        if (labelNode) labelNode.textContent = isOpen ? openLabel : closedLabel;
