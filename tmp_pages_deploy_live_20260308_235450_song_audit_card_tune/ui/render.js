...
let lyricsEditorLineMeasureNode = null;

function ensureLyricsEditorLineMeasureNode() {
  if (lyricsEditorLineMeasureNode instanceof HTMLElement) return lyricsEditorLineMeasureNode;
  ...
  lyricsEditorLineMeasureNode = node;
  return node;
}

function resolveTextareaVisualLineSpan(textareaNode, text = "", options = {}) {
  if (!(textareaNode instanceof HTMLTextAreaElement)) return 1;
  const style = window.getComputedStyle(textareaNode);
  const lineHeightPx = Math.max(18, Number(options?.lineHeightPx || Number.parseFloat(String(style.lineHeight || "")) || 24));
  const paddingLeftPx = Math.max(0, Number.parseFloat(String(style.paddingLeft || "")) || 0);
  const paddingRightPx = Math.max(0, Number.parseFloat(String(style.paddingRight || "")) || 0);
  const contentWidthPx = Math.max(
    1,
    Number(options?.contentWidthPx || Math.round(textareaNode.clientWidth - paddingLeftPx - paddingRightPx)),
  );
  const measureNode = ensureLyricsEditorLineMeasureNode();
  measureNode.style.width = `${contentWidthPx}px`;
  measureNode.style.fontFamily = style.fontFamily || "inherit";
  measureNode.style.fontSize = style.fontSize || "16px";
  measureNode.style.fontWeight = style.fontWeight || "400";
  measureNode.style.fontStyle = style.fontStyle || "normal";
  measureNode.style.letterSpacing = style.letterSpacing || "normal";
  measureNode.style.lineHeight = `${lineHeightPx}px`;
  measureNode.style.textTransform = style.textTransform || "none";
  measureNode.style.textIndent = style.textIndent || "0px";
  measureNode.style.tabSize = style.tabSize || "8";
  measureNode.textContent = String(text || "").length ? String(text) : "\u00A0";
  const measuredHeight = Math.max(lineHeightPx, Math.ceil(measureNode.getBoundingClientRect().height));
  return Math.max(1, Math.ceil((measuredHeight + 0.5) / lineHeightPx));
}
    ...
    const initRequestLineAssistant = () => {
      ...
      const renderRail = () => {
        ...
        const contentWidthPx = Math.max(
          1,
          Math.round(
            Number(lyricsTextareaNode.clientWidth || 0)
            - resolvePaddingPx(textareaStyle.paddingLeft, 0)
            - resolvePaddingPx(textareaStyle.paddingRight, 0),
          ),
        );
        ...
        lyricsLineRailListNode.innerHTML = sourceLines.map((lineText, index) => {
          ...
          const span = resolveTextareaVisualLineSpan(lyricsTextareaNode, lineText, { lineHeightPx, contentWidthPx });
          const slots = [];
          for (let slotIndex = 0; slotIndex < span; slotIndex += 1) {
            ...
            slots.push(`
              <div class="ac-lyrics-rail-slot has-confidence ${confidenceBand}" data-line-id="${esc(lineId)}" data-line-index="${index}" data-slot-index="${slotIndex}" style="${draftConfidenceStyleValue(confidence)}">
  ...
  if ((route.name === "admin" && ctx.section === "editor") || route.name === "draft") {
    ...
    const inlineLineVisualSpan = (line, options = {}) => {
      ...
      const text = String(options?.text ?? fallbackText)
        ...
        .replace(/^\n+|\n+$/g, "");
      if (!text) return 1;
      return resolveTextareaVisualLineSpan(lyricsTextareaNode, text, {
    ...
    const inlineRenderLineRail = () => {
      ...
      const contentWidthPx = Math.max(
        ...
        Math.round(
          Number(lyricsTextareaNode.clientWidth || 0)
          - inlineResolvePaddingPx(textareaStyle.paddingLeft, 0)
          - inlineResolvePaddingPx(textareaStyle.paddingRight, 0),
        ),
      );
      const contentHeightPx = Math.max(
        Math.round(Number(lyricsTextareaNode.scrollHeight || 0)),
      ...
      lyricsLineRailListNode.innerHTML = lines.map((line, index) => {
        ...
        const span = inlineLineVisualSpan(line, { text: sourceText, lineHeightPx, contentWidthPx });
        const slots = [];
        for (let slotIndex = 0; slotIndex < span; slotIndex += 1) {
