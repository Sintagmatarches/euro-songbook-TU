...
function syncLyricsEditorOverlay(editorNode, textareaNode, sourceLines = [], lines = [], options = {}) {
  ...
  overlayContentNode.style.whiteSpace = textWhiteSpace;
  overlayContentNode.style.overflowWrap = textOverflowWrap;
  overlayContentNode.style.wordBreak = textWordBreak;
  overlayContentNode.style.width = wrapsText ? "100%" : "max-content";
  overlayContentNode.style.minWidth = "100%";
  overlayContentNode.style.maxWidth = wrapsText ? "100%" : "none";
...
function resolveLyricsEditorOverlayLineMetrics(editorNode, options = {}) {
  ...
  const overlayContentNode = editorNode.querySelector(".ac-lyrics-overlay-content");
  const overlayLines = overlayContentNode instanceof HTMLElement
    ? Array.from(overlayContentNode.querySelectorAll(".ac-lyrics-overlay-line"))
    : [];
  if (!overlayLines.length) {
    return Array.from({ length: fallbackCount }, (_, index) => ({
      top: Math.round(fallbackPadTopPx + (index * fallbackLineHeightPx)),
      height: Math.round(fallbackLineHeightPx),
    }));
  }
  const metrics = overlayLines.map((lineNode, index) => ({
    top: Math.round(Number(lineNode?.offsetTop || fallbackPadTopPx + (index * fallbackLineHeightPx))),
    height: Math.max(1, Math.round(Number(lineNode?.offsetHeight || fallbackLineHeightPx))),
  }));
  while (metrics.length < fallbackCount) {
    metrics.push({
      top: Math.round(fallbackPadTopPx + (metrics.length * fallbackLineHeightPx)),
      height: Math.round(fallbackLineHeightPx),
    });
  }
  return metrics;
  ...
  if (route.name === "request") {
    ...
    const initRequestLineAssistant = () => {
      ...
      const renderRail = () => {
        ...
        const overlayMetrics = resolveLyricsEditorOverlayLineMetrics(lyricsEditorNode, {
        ...
        lyricsLineRailListNode.innerHTML = sourceLines.map((lineText, index) => {
          ...
          const metric = overlayMetrics[index] || {
            top: Math.round(padTopPx + (index * lineHeightPx)),
            height: Math.round(lineHeightPx),
          };
          const slotLayoutStyle = `top:${Math.max(0, metric.top)}px;height:${Math.max(1, metric.height)}px;min-height:${Math.max(1, metric.height)}px;`;
