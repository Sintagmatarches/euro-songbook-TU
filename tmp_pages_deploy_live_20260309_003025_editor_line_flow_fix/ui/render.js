...
function draftLinePopoverUI(line = null, index = -1) {
  ...
  const allowManualChoice = draftLineHasConfidenceTie(normalizedLine);
  const safeIndex = Math.max(0, Number(index || 0));
  return `
    <div class="ac-line-popover-card ${draftConfidenceBand(activeConfidence)}" data-line-id="${esc(normalizedLine.id)}" style="${draftConfidenceStyleValue(activeConfidence)}">
      ...
      <div class="draft-variants-panel">
        ${variants.map((variant, variantIndex) => {
          ...
          const canActivateManually = allowManualChoice
            && !isActive
            && Number(variantConfidence) === Number(activeConfidence);
...
function renderTextWithConfidenceWords(text, segments = [], options = {}) {
  ...
  const variantPanelMarkup = String(options?.variantPanelMarkup || "");
  ...
  for (const word of words) {
    ...
    if (confidence >= 100) {
      ...
      const wordMarkup = `<span class="song-confidence-word" style="${confidenceStyle}">${renderTextWithUnknownMarkers(wordText)}</span>`;
      if (variantPanelMarkup) {
        out += `<span class="song-confidence-word-anchor" style="${confidenceStyle}"><details class="song-confidence-word-menu"><summary class="song-confidence-word-toggle" aria-label="${esc(variantToggleTitle || confidenceLabel)}" title="${esc(variantToggleTitle || confidenceLabel)}"><span>${esc(confidenceLabel)}</span></summary>${variantPanelMarkup}</details>${wordMarkup}</span>`;
      } else {
...
function renderLyricsEditorMeasureLines(sourceLines = []) {
  ...
  return safeSourceLines.map((sourceText) => {
    ...
    return `<div class="ac-lyrics-measure-line">${content ? esc(content) : "&nbsp;"}</div>`;
  }).join("");
}
...
function resolveLyricsEditorLineMetrics(editorNode, options = {}) {
  ...
  const metrics = measureLines.map((lineNode, index) => {
    ...
    const top = Number.isFinite(lineRect?.top)
      ? Math.max(0, Number(lineRect.top) - Number(contentRect.top || 0))
      : fallbackPadTopPx + (index * fallbackLineHeightPx);
    const height = Number.isFinite(lineRect?.height) && Number(lineRect.height) > 0
      ? Number(lineRect.height)
  ...
  const variantsMarkup = variants.length
    ? variants.map((variant) => `
      <li class="song-line-variant-row">
        <span class="song-line-variant-text">${String(variant?.text || "").trim() ? renderTextWithUnknownMarkers(String(variant?.text || "")) : "&mdash;"}</span>
      </li>
    `).join("")
...
function renderLyricLineWithConfidence(text, meta = null) {
  ...
  const variantsPanel = renderSongLyricVariantsPanel(safeMeta);
  const hasVariantMenu = !!variantsPanel;
  ...
  return `<div class="${classes}"><div class="song-lyric-line-text">${content ? renderTextWithConfidenceWords(content, safeSegments, {
    variantPanelMarkup: hasVariantMenu ? variantsPanel : "",
  ...
  if (route.name === "request") {
    ...
    const initRequestLineAssistant = () => {
      ...
      const renderRail = () => {
        ...
        const padLeftPx = resolvePaddingPx(textareaStyle.paddingLeft, 0);
        const padRightPx = resolvePaddingPx(textareaStyle.paddingRight, 0);
        ...
        const railWidthPx = Math.max(10, Math.round(Number(lyricsLineRailListNode.parentElement?.clientWidth || 20)));
        const ghostOffsetPx = railWidthPx + 4;
        const ghostWidthPx = Math.max(
          1,
          Math.round(Number(lyricsTextareaNode.clientWidth || lyricsTextareaNode.offsetWidth || 0)) - padLeftPx - padRightPx,
        );
        ...
        lyricsLineRailListNode.style.whiteSpace = String(textareaStyle.whiteSpace || "").trim() || "pre";
        lyricsLineRailListNode.style.overflowWrap = String(textareaStyle.overflowWrap || "").trim() || "normal";
        lyricsLineRailListNode.style.wordBreak = String(textareaStyle.wordBreak || "").trim() || "normal";
        lyricsLineRailListNode.style.setProperty("--ac-rail-ghost-width", formatCssPx(ghostWidthPx, 1));
        lyricsLineRailListNode.style.setProperty("--ac-rail-ghost-offset", formatCssPx(ghostOffsetPx, railWidthPx + 4));
        lyricsLineRailListNode.style.paddingTop = formatCssPx(padTopPx, 12);
        lyricsLineRailListNode.style.paddingBottom = formatCssPx(padBottomPx, 12);
        ...
        lyricsLineRailListNode.innerHTML = sourceLines.map((lineText, index) => {
          ...
          const ghostText = String(lineText ?? "");
          const ghostMarkup = ghostText ? esc(ghostText) : "&nbsp;";
          if (isTextLine) visibleLineNumber += 1;
          if (!isTextLine) {
            return `<div class="ac-lyrics-rail-slot is-empty" data-line-index="${index}" data-slot-index="0" aria-hidden="true"><div class="ac-lyrics-rail-row"><span class="draft-line-rail-btn-placeholder" aria-hidden="true"></span></div><div class="ac-lyrics-rail-ghost" aria-hidden="true">${ghostMarkup}</div></div>`;
          }
          ...
          return `
            <div class="ac-lyrics-rail-slot has-confidence ${confidenceBand}" data-line-id="${esc(lineId)}" data-line-index="${index}" data-slot-index="0" style="${draftConfidenceStyleValue(confidence)}">
              ...
              <div class="ac-lyrics-rail-ghost" aria-hidden="true">${ghostMarkup}</div>
            </div>
          `;
        }).join("");
  ...
  if ((route.name === "admin" && ctx.section === "editor") || route.name === "draft") {
    ...
    const inlineIsEditingVariantInput = () => {
      ...
      return active.classList.contains("draft-variant-text-input")
        || active.classList.contains("draft-variant-confidence-select")
        || active.classList.contains("draft-variant-confidence-level")
        || active.classList.contains("draft-variant-confidence-input")
        || active.classList.contains("draft-word-confidence-select")
        || active.classList.contains("draft-word-confidence-input")
    ...
    const inlineRenderLineRail = () => {
      ...
      const padLeftPx = inlineResolvePaddingPx(textareaStyle.paddingLeft, 0);
      const padRightPx = inlineResolvePaddingPx(textareaStyle.paddingRight, 0);
      const contentHeightPx = Math.max(
        Math.round(Number(lyricsTextareaNode.scrollHeight || 0)),
        Math.round(Number(lyricsTextareaNode.offsetHeight || lyricsTextareaNode.clientHeight || 0)),
        Math.round(lineHeightPx * 2),
      ...
      lyricsLineRailListNode.style.overflowWrap = String(textareaStyle.overflowWrap || "").trim() || "normal";
      lyricsLineRailListNode.style.wordBreak = String(textareaStyle.wordBreak || "").trim() || "normal";
      lyricsLineRailListNode.style.setProperty("--ac-rail-ghost-width", inlineFormatCssPx(ghostWidthPx, 1));
      lyricsLineRailListNode.style.setProperty("--ac-rail-ghost-offset", inlineFormatCssPx(ghostOffsetPx, railWidthPx + 4));
      lyricsLineRailListNode.style.paddingTop = inlineFormatCssPx(padTopPx, 12);
      lyricsLineRailListNode.style.paddingBottom = inlineFormatCssPx(padBottomPx, 12);
      ...
      lyricsLineRailListNode.innerHTML = lines.map((line, index) => {
        ...
        const ghostMarkup = sourceText ? esc(sourceText) : "&nbsp;";
        ...
        return `
          <div class="ac-lyrics-rail-slot ${hasText ? `has-confidence ${confidenceBand}` : "is-empty"}" data-line-id="${esc(lineId)}" data-line-index="${index}" data-slot-index="0"${hasText ? ` style="${draftConfidenceStyleValue(confidence)}"` : ""}>
            <div class="ac-lyrics-rail-row ${confidenceBand} ${isActive ? "is-active" : ""}">
              ${buttonMarkup}
            </div>
            <div class="ac-lyrics-rail-ghost" aria-hidden="true">${ghostMarkup}</div>
