...
function draftResolveLinePopoverMetrics(editorRect, textRect, anchorRect, popoverNode) {
  ...
  const minPopoverWidth = isCompactMobile ? 196 : 264;
  const availableWidth = Math.max(minPopoverWidth, viewportWidth - horizontalInset);
  const maxPopoverWidth = Math.max(minPopoverWidth, Math.min(availableWidth, isCompactMobile ? 296 : 420));
  const desiredWidth = isCompactMobile
    ? Math.round(Math.min(editorRect.width - 28, Math.max(minPopoverWidth, textRect.width * 0.58)))
  ...
  const baseLeft = Math.max(0, Math.round(anchorRect.left - editorRect.left + anchorOffset - (isCompactMobile ? 4 : 0)));
  ...
  const mobileHeightCap = Math.max(220, Math.min(Math.floor(viewportHeight - 92), 312));
