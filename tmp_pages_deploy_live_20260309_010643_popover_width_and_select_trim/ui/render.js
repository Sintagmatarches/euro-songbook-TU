...
function draftResolveLinePopoverMetrics(editorRect, textRect, anchorRect, popoverNode) {
  ...
  const minPopoverWidth = isCompactMobile ? 224 : 264;
  const availableWidth = Math.max(minPopoverWidth, viewportWidth - horizontalInset);
  const maxPopoverWidth = Math.max(minPopoverWidth, Math.min(availableWidth, isCompactMobile ? Math.round(editorRect.width - 8) : 420));
  ...
  const desiredWidth = isCompactMobile
    ? Math.round(Math.min(editorRect.width - mobileBaseLeft - 4, Math.max(minPopoverWidth, (textRect.width + 4) * 0.75)))
