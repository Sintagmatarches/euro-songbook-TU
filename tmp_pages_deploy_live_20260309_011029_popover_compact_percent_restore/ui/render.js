...
function draftResolveLinePopoverMetrics(editorRect, textRect, anchorRect, popoverNode) {
  ...
  const horizontalInset = isCompactMobile ? 14 : 24;
  const minPopoverWidth = isCompactMobile ? 176 : 264;
  const availableWidth = Math.max(minPopoverWidth, viewportWidth - horizontalInset);
  const maxPopoverWidth = Math.max(minPopoverWidth, Math.min(availableWidth, isCompactMobile ? 244 : 420));
  const anchorOffset = Math.max(18, Math.round(anchorRect.width || 0) + 6);
  const mobileBaseLeft = Math.max(0, Math.round(textRect.left - editorRect.left - 2));
  ...
  const desiredWidth = isCompactMobile
    ? Math.round(Math.min(editorRect.width - mobileBaseLeft - 8, Math.max(minPopoverWidth, textRect.width * 0.68)))
  ...
  const mobileHeightCap = Math.max(260, Math.min(Math.floor(viewportHeight - 56), 520));
  let openHeight = Math.min(measuredHeight, belowSpace, mobileHeightCap);
  let top = belowTop;

  if (measuredHeight > belowSpace && aboveSpace > belowSpace) {
    openHeight = Math.min(measuredHeight, aboveSpace, mobileHeightCap);
