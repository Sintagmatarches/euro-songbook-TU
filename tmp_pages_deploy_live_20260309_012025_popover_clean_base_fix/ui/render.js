...
function draftResolveLinePopoverMetrics(editorRect, textRect, anchorRect, popoverNode) {
  ...
  const horizontalInset = isCompactMobile ? 18 : 24;
  ...
  const maxPopoverWidth = Math.max(
    ...
    Math.min(
      availableWidth,
      isCompactMobile ? Math.round(Math.min(editorRect.width - 12, Math.max(minPopoverWidth, textRect.width * 0.82))) : 420,
    ),
  );
  const desiredWidth = isCompactMobile
    ? Math.round(Math.min(editorRect.width - 12, Math.max(minPopoverWidth, textRect.width * 0.76)))
