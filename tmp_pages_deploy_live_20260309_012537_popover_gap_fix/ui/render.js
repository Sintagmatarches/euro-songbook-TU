...
function draftResolveLinePopoverMetrics(editorRect, textRect, anchorRect, popoverNode) {
  ...
  const horizontalInset = isCompactMobile ? 26 : 24;
  const minPopoverWidth = isCompactMobile ? 168 : 264;
  const availableWidth = Math.max(minPopoverWidth, viewportWidth - horizontalInset);
  const maxPopoverWidth = Math.max(
    minPopoverWidth,
    Math.min(
      availableWidth,
      isCompactMobile ? Math.round(Math.min(editorRect.width - 18, Math.max(minPopoverWidth, textRect.width * 0.62))) : 420,
    ),
  );
  const desiredWidth = isCompactMobile
    ? Math.round(Math.min(editorRect.width - 18, Math.max(minPopoverWidth, textRect.width * 0.56)))
  ...
  const mobileBaseLeft = Math.max(0, Math.round(textRect.left - editorRect.left));
