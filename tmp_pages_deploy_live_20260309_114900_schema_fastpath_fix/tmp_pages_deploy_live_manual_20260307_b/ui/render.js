    ...
    const variants = Array.isArray(line?.variants)
      ? line.variants.map((variant, variantIndex) => {
        ...
        const variantConfidence = draftClampConfidence(
          variant?.confidence,
          variant?.is_active ? (confidence ?? 100) : 80,
        );
        return {
          ...
          is_active: variant?.is_active === true || String(variant?.id || "") === String(line?.active_variant_id || ""),
        };
      })
      : [];
    return {
...
function songLyricLineStyleValue(confidence) {
  const safeConfidence = draftClampConfidence(confidence, 100);
  const details = confidenceLevelDetails(draftConfidenceLevel(safeConfidence));
  const strength = Math.max(0, Math.min(1, Number(details.fill || 0) / 100));
  const darkAlpha = 0.34 - (strength * 0.16);
  const greenAlpha = 0.08 + (strength * 0.26);
  const borderAlpha = 0.18 + (strength * 0.18);
  return [
    `--song-line-fill:${Math.round(Number(details.fill || 0))}%`,
    `--song-line-dark-alpha:${darkAlpha.toFixed(3)}`,
    `--song-line-green-alpha:${greenAlpha.toFixed(3)}`,
    `--song-line-border-alpha:${borderAlpha.toFixed(3)}`,
  ].join(";");
}
...
function renderSongLyricVariantsPanel(meta = null) {
  ...
  const activeSegments = draftNormalizeSegments(safeMeta?.confidence_segments, safeMeta?.text || "");
  if (variants.length <= 1 && !activeSegments.length) return "";
    ...
    ? variants.map((variant) => `
      <div class="song-line-variant-row ${variant?.is_active ? "is-active" : ""}">
        ...
        <div class="song-line-variant-meta">
          ...
          <span class="badge">${esc(draftConfidenceLabel(variant?.confidence_level || variant?.confidence || 100))}</span>
        </div>
      </div>
    `).join("")
    : `<div class="muted small">${esc(draftUiText("confidenceSegmentsEmpty"))}</div>`;

  const segmentsMarkup = activeSegments.length
    ? `
      <div class="song-line-variant-segments">
        <div class="song-line-variant-segments-title">${esc(draftUiText("confidenceSegmentsTitle"))}</div>
        <div class="song-line-variant-segments-list">
          ${activeSegments.map((segment) => `
            <span class="song-line-segment-chip ${draftConfidenceBand(segment.level)}" style="${draftConfidenceStyleValue(segment.level)}">
              <span class="song-line-segment-text">${esc(draftSegmentSnippet(safeMeta?.text || "", segment) || "...")}</span>
              <span class="song-line-segment-level">${esc(draftConfidenceLabel(segment.level))}</span>
            </span>
          `).join("")}
        </div>
      </div>
  ...
  const confidenceBadge = Number.isFinite(Number(safeMeta?.confidence)) && Number(safeMeta?.confidence) < 100
    ? `<span class="badge">${esc(`${draftClampConfidence(safeMeta.confidence, 100)}%`)}</span>`
    : "";
...
function renderLyricLineWithConfidence(text, meta = null) {
  ...
  const classes = [
    ...
    (hasVisibleConfidence || hasVariantMenu) ? "has-tools" : "",
    hasVariantMenu ? "has-variants" : "",
    publicBand,
  ].filter(Boolean).join(" ");
  const content = String(text || "");
  const controlsMarkup = (hasVisibleConfidence || hasVariantMenu)
    ? `
      <div class="song-lyric-line-tools">
        ${hasVisibleConfidence ? `<span class="song-lyric-line-confidence" aria-hidden="true">${safeConfidence}%</span>` : ""}
        ${hasVariantMenu ? `
    ...
    `
    : "";
  const styleAttr = hasVisibleConfidence ? ` style="${songLyricLineStyleValue(safeConfidence)}"` : "";
  const confidenceAttr = hasVisibleConfidence ? ` data-confidence="${safeConfidence}"` : "";
  return `<div class="${classes}"${confidenceAttr}${styleAttr}><div class="song-lyric-line-text">${content ? renderTextWithUnknownMarkers(content) : "&nbsp;"}</div>${controlsMarkup}</div>`;
}
...
function renderStructuredLyrics(rawLyrics, options = {}) {
  ...
  const hasVisibleStructuredConfidence = renderLines.some((item) => {
    ...
    return safeConfidence < 100 || segments.length > 0 || variants.length > 1;
  });
