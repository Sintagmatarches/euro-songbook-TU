...
function renderSongLyricVariantsPanel(meta = null) {
  ...
  const variantsMarkup = variants.length
    ? variants.map((variant, variantIndex) => {
      ...
      const variantText = String(variant?.text || "");
      const variantConfidence = draftClampConfidence(variant?.confidence, isActive ? activeConfidence : 80);
      const confidenceLabel = draftConfidencePercent(variantConfidence, 100);
      const confidenceStyle = songConfidenceWordStyleValue(variantConfidence);
      return `
      <li class="song-line-variant-row ${isActive ? "is-active" : ""}" style="${confidenceStyle}">
        <span class="song-line-variant-main">
          <span class="song-line-variant-text">${variantText.trim() ? renderTextWithUnknownMarkers(variantText) : "&mdash;"}</span>
          <span class="song-line-variant-confidence" aria-label="${esc(confidenceLabel)}" title="${esc(confidenceLabel)}">${esc(confidenceLabel)}</span>
        </span>
      </li>
    `;
    }).join("")
