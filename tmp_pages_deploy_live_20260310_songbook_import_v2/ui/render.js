        ...
        <div class="card songbook-import-main" id="songbookImportRoot">
          ...
          <div class="grid2 songbook-import-top-grid">
            <label class="field">
              ...
              <input class="input" id="sb_import_lang" value="${esc(selectedImportLang)}" placeholder="bel" />
            </label>
            <label class="field">
  ...
  const result = await Tesseract.recognize(source, ocrLang, {
    logger: (message) => {
      ...
      if (message?.status === "recognizing text" && Number.isFinite(Number(message?.progress))) {
        ...
        options.onProgress(`${options.progressPrefix || "OCR"} ${percent}%`);
      }
    },
  });
  return songbookImportTextFromOcrResult(result);
}
...
async function songbookImportExtractPdfText(arrayBuffer, options = {}) {
  ...
  for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
    ...
    if ((!pageText || pageText.length < 24) && mode !== "text") {
      pageText = await songbookImportOcrPdfPage(page, {
