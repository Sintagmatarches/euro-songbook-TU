...
import { deleteSongbookImportSession, getSongbookImportSession, saveSongbookImportSession } from "../../../_lib/songbook-imports.js";
...
export async function onRequestDelete({ env, request, params }) {
  ...
  if (!sessionId) return err("Session id is required", 400);
  try {
    const out = await deleteSongbookImportSession(env, sessionId);
    return json(out);
  } catch (cause) {
    return err(cause?.message || "Failed to delete import session", 400);
  }
}
