...
function adminSongbookImportsUI(data, params = {}) {
  ...
  const selectedImportLang = String(selected?.import_lang || "bel+rus").trim() || "bel+rus";
        ...
        <div class="card songbook-import-main" id="songbookImportRoot">
          ...
          <div class="grid2 songbook-import-top-grid">
            <label class="field">
              ...
              <input class="input" id="sb_import_lang" value="${esc(selectedImportLang)}" placeholder="bel+rus"${isSubmitted ? " readonly" : ""} />
            </label>
            <label class="field">
...
function songbookImportNormalizeOcrLang(value = "") {
  const normalized = String(value || "").trim().toLowerCase();
  if (!normalized) return "bel+rus";
  if (normalized === "bel") return "bel+rus";
  return normalized;
}
...
async function songbookImportPrepareOcrCanvas(source, options = {}) {
  ...
  context.filter = "grayscale(1) contrast(185%) brightness(112%)";
  context.drawImage(
    sourceCanvas,
    crop.left,
    crop.top,
  ...
  const threshold = Math.max(70, Math.min(210, Math.round(otsu * (options.binarize ? 0.94 : 1))));

  for (let index = 0, pixel = 0; index < data.length; index += 4, pixel += 1) {
    let normalized = (lumaValues[pixel] - min) / dynamicRange;
    normalized = Math.max(0, Math.min(1, ((normalized - 0.5) * 1.55) + 0.5));
...
async function songbookImportRecognizeImage(source, options = {}) {
  ...
  const variants = [
    { id: "grayscale", binarize: false },
    { id: "binary", binarize: true },
  ];
  let bestText = "";
  let bestScore = Number.NEGATIVE_INFINITY;
  for (let index = 0; index < variants.length; index += 1) {
    const variant = variants[index];
    const prepared = await songbookImportPrepareOcrCanvas(source, variant);
