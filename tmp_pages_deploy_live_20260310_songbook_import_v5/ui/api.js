...
const PERSISTENT_GET_CACHE_KEY = "songbook_api_get_cache_v1";
...
async function req(path, opts = {}) {
  ...
  let res;
  try {
    res = await fetch(absoluteUrl, { ...opts, headers, method });
  } catch (error) {
    if (canUseCache) {
...
export const api = {
  ...
  async adminSongbookImports() {
    return req("api/admin/songbook-imports");
  },
  async adminSongbookImport(id) {
    return req(`api/admin/songbook-imports/${encodeURIComponent(id)}`);
  },
  async adminDeleteSongbookImport(id) {
    return req(`api/admin/songbook-imports/${encodeURIComponent(id)}`, {
      method: "DELETE",
    });
  },
  async adminSaveSongbookImport(payload = {}) {
    const sessionId = String(payload?.id || "").trim();
    if (sessionId) {
      return req(`api/admin/songbook-imports/${encodeURIComponent(sessionId)}`, {
        method: "PUT",
  ...
  async adminSubmitSongbookImport(id) {
    return req(`api/admin/songbook-imports/${encodeURIComponent(id)}/submit`, {
      method: "POST",
    });
  },
