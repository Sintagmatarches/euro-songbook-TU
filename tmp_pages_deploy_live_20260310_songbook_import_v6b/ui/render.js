...
function adminTabs(active) {
  ...
  const usersTabLabel = uiLocale() === "ru"
    ...
    : uiLocale() === "uk"
      ...
      : uiLocale() === "et"
        ? "Kasut."
        : "Users";
  const importsTabLabel = songbookImportUiText("tab");
  ...
  return `
    <div class="admin-tabs" style="margin-bottom:12px">
      ...
      ${canEditSongs ? `<a class="btn admin-tab-btn ${active === "imports" ? "primary" : "ghost"}" href="#/admin/imports">${esc(importsTabLabel)}</a>` : ``}
  ...
  if (route.name === "admin") {
    ...
    if (section === "content") {
      ...
      return {
        ...
        ctx: { section, data, params: { ...params, recent: params.recent || "", sort: params.sort || "newest" } },
      };
    }
    ...
    if (section === "imports") {
      if (!canContent) {
        const deniedText = uiLocale() === "ru"
          ? "Недостаточно прав для импорта песенников."
          : uiLocale() === "uk"
            ? "Недостатньо прав для імпорту пісенників."
            : uiLocale() === "et"
              ? "Laulikute importimiseks pole piisavalt oigusi."
              : "Not enough permissions to import songbooks.";
        return { html: `${adminTabs("imports")}<div class="card"><div class="muted">${esc(deniedText)}</div></div>`, ctx: { section } };
      }
      const sessionId = String(params.id || "").trim();
      const [itemsPayload, selectedPayload] = await Promise.all([
        api.adminSongbookImports(),
        sessionId ? api.adminSongbookImport(sessionId).catch(() => ({ item: null })) : Promise.resolve({ item: null }),
      ]);
      const data = {
        items: Array.isArray(itemsPayload?.items) ? itemsPayload.items : [],
        selected: selectedPayload?.item || null,
      };
      return { html: adminSongbookImportsUI(data, params), ctx: { section, data, params } };
    }
...
export function bind(route, ctx) {
  ...
  if (route.name === "admin" && ctx.section === "imports") {
    const fileInput = qs("sb_file");
    const sessionIdInput = qs("sb_session_id");
    const sessionStatusInput = qs("sb_session_status");
    const sourceNameInput = qs("sb_source_name");
    const importLangInput = qs("sb_import_lang");
    const importCountryInput = qs("sb_import_country");
    const extractionModeSelect = qs("sb_extraction_mode");
    const editorText = qs("sb_editor_text");
    const rawText = qs("sb_raw_text");
    const statsNode = qs("sb_stats");
    const statusHintNode = qs("sb_status_hint");
    const previewList = qs("sb_preview_list");
    const extractBtn = qs("sb_extract");
    const insertMarkerBtn = qs("sb_insert_marker");
    const saveBtn = qs("sb_save");
    const submitBtn = qs("sb_submit");
    const deleteBtn = qs("sb_delete");
    const editCopyBtn = qs("sb_edit_copy");
    if (!editorText || !rawText || !previewList) return;

    wireAutoGrowTextareas(document.getElementById("songbookImportRoot") || document.body);
    let currentSessionStatus = String(sessionStatusInput?.value || "draft").trim() || "draft";
    let isEditableSession = currentSessionStatus !== "submitted";

    const applyEditability = () => {
      isEditableSession = currentSessionStatus !== "submitted";
      if (sourceNameInput) sourceNameInput.readOnly = !isEditableSession;
      if (importLangInput) importLangInput.readOnly = !isEditableSession;
      if (importCountryInput) importCountryInput.readOnly = !isEditableSession;
      if (extractionModeSelect) extractionModeSelect.disabled = !isEditableSession;
      if (fileInput) fileInput.disabled = !isEditableSession;
      editorText.readOnly = !isEditableSession;
      rawText.readOnly = true;
      [extractBtn, insertMarkerBtn, saveBtn, submitBtn].forEach((node) => {
        if (node) node.disabled = !isEditableSession;
      });
      if (statusHintNode) {
        statusHintNode.textContent = isEditableSession
          ? songbookImportUiText("sessionEditableHint")
          : songbookImportUiText("sessionReadonlyHint");
      }
    };
    ...
    const updatePreview = () => {
      const split = splitSongbookImportText(editorText.value || "");
      if (statsNode) {
        statsNode.textContent = songbookImportUiText("stats", {
          songCount: split.songCount,
          markerCount: split.markerCount,
        });
      }
      previewList.innerHTML = split.songs.length
        ? songbookImportPreviewCards(split.songs)
        : `<div class="muted">${esc(songbookImportUiText("noPreview"))}</div>`;
      return split;
    };

    const collectPayload = () => ({
      id: sessionIdInput?.value || "",
      source_name: sourceNameInput?.value || "",
      import_lang: importLangInput?.value || "",
      import_country: importCountryInput?.value || "",
      extraction_mode: extractionModeSelect?.value || "auto",
    ...
    const setBusy = (busy, message = "") => {
      [extractBtn, saveBtn, submitBtn, insertMarkerBtn, deleteBtn, editCopyBtn].forEach((node) => {
        if (node) node.disabled = !!busy;
      });
      if (statsNode && message) statsNode.textContent = message;
      if (!busy) applyEditability();
      if (statsNode && !busy && !message) updatePreview();
    };

    const persistSession = async (options = {}) => {
      const payload = collectPayload();
      const out = await api.adminSaveSongbookImport(payload);
      const item = out?.item || null;
      if (!item?.id) throw new Error("Failed to save import session");
      if (sessionIdInput) sessionIdInput.value = item.id;
      currentSessionStatus = String(item.status || "draft").trim() || "draft";
      if (sessionStatusInput) sessionStatusInput.value = currentSessionStatus;
      if (options.showToast !== false) {
        showStatusOverlay(songbookImportUiText("saveDone"), "success");
      }
      if (options.navigate !== false) {
        const targetHash = `#/admin/imports?id=${encodeURIComponent(item.id)}`;
    ...
    importLangInput?.addEventListener("input", () => updatePreview());

    extractBtn?.addEventListener("click", async () => {
      if (!isEditableSession) return;
      const file = fileInput?.files?.[0];
      if (!file) {
        showStatusOverlay(songbookImportUiText("fileRequired"), "error");
        return;
      }
      ...
      try {
        setBusy(true, songbookImportUiText("extractionProgress"));
        const extracted = await songbookImportExtractTextFromFile(file, {
          ocrLang: importLangInput?.value || "bel",
          extractionMode: extractionModeSelect?.value || "auto",
          onProgress: (label) => {
            if (statsNode) statsNode.textContent = String(label || "");
          },
        });
        const cleaned = songbookImportCleanupExtractedText(extracted, {
          ocrLang: importLangInput?.value || "bel",
        });
        rawText.value = extracted;
        editorText.value = cleaned || extracted;
        updatePreview();
      } catch (error) {
        showStatusOverlay(`${songbookImportUiText("loadError")} ${error?.message || ""}`.trim(), "error");
      } finally {
        setBusy(false);
      }
    });
    ...
    insertMarkerBtn?.addEventListener("click", () => {
      if (!isEditableSession) return;
      const markerBlock = `\n${SONGBOOK_IMPORT_MARKER}\n`;
      const start = editorText.selectionStart ?? editorText.value.length;
      const end = editorText.selectionEnd ?? editorText.value.length;
      editorText.setRangeText(markerBlock, start, end, "end");
      editorText.focus();
      updatePreview();
    });
    ...
    saveBtn?.addEventListener("click", async () => {
      if (!isEditableSession) return;
      try {
        setBusy(true, songbookImportUiText("save"));
    ...
    submitBtn?.addEventListener("click", async () => {
      if (!isEditableSession) return;
      try {
        setBusy(true, songbookImportUiText("submit"));
        const item = await persistSession({ navigate: false, showToast: false });
        const out = await api.adminSubmitSongbookImport(item.id);
        showStatusOverlay(songbookImportUiText("submitDone", { count: out?.submitted_count || 0 }), "success");
        location.hash = `#/admin/imports?id=${encodeURIComponent(item.id)}`;
      } catch (error) {
        showStatusOverlay(`${t("common.error")}: ${error?.message || t("common.error")}`, "error");
      } finally {
        setBusy(false);
    ...
    editCopyBtn?.addEventListener("click", () => {
      ...
      if (sessionStatusInput) sessionStatusInput.value = "draft";
      applyEditability();
      showStatusOverlay(songbookImportUiText("editCopyDone"), "success");
    });

    deleteBtn?.addEventListener("click", async () => {
      const sessionId = String(sessionIdInput?.value || "").trim();
      if (!sessionId) return;
      if (!confirm(songbookImportUiText("deleteConfirm"))) return;
      try {
        setBusy(true, songbookImportUiText("delete"));
        await api.adminDeleteSongbookImport(sessionId);
        showStatusOverlay(songbookImportUiText("deleteDone"), "success");
        location.hash = "#/admin/imports";
      } catch (error) {
        showStatusOverlay(`${t("common.error")}: ${error?.message || t("common.error")}`, "error");
      } finally {
        setBusy(false);
