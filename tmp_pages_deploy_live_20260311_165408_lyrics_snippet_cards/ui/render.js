...
import {
  ...
  getHistoricalGroupLabel,
  getHistoricalGroupOptions,
...
function historicalGroupFieldLabel() {
  if (uiLocale() === "ru") return "Историческая группа";
  if (uiLocale() === "uk") return "Історична група";
  if (uiLocale() === "et") return "Ajalooline grupp";
  return "Historical group";
}
...
function historicalGroupPlaceholderText() {
  if (uiLocale() === "ru") return "Выберите группу";
  if (uiLocale() === "uk") return "Оберіть групу";
  if (uiLocale() === "et") return "Vali grupp";
  return "Select group";
}
...
function historicalGroupSelectOptions(lang, currentGroup = "", currentCountry = "") {
  ...
  const selected = String(currentGroup || resolved.group_key || "").trim();
  const options = safeLang || currentCountry
    ? getHistoricalGroupOptions(safeLang, uiLocale(), { currentCountry })
    : [];
  return [
    `<option value="">${esc(historicalGroupPlaceholderText())}</option>`,
    ...options.map((opt) => `<option value="${esc(opt.value)}" ${selected === opt.value ? "selected" : ""}>${esc(opt.label)}</option>`),
  ].join("");
}
...
function historicalAffiliationSelectOptions(lang, groupKey = "", currentCountry = "") {
  const resolved = resolveHistoricalAffiliationSelection(currentCountry);
  const safeLang = normalizeSongLanguage(lang || resolved.lang || "") || "";
  const safeGroup = String(groupKey || resolved.group_key || "").trim();
  const selectedCountry = normalizeSongCountry(currentCountry || "") || "";
  const options = safeLang || safeGroup || selectedCountry
    ? getHistoricalAffiliationOptions(safeLang, safeGroup, uiLocale(), { currentCountry: selectedCountry })
    : [];
  return [
...
function setHistoricalSelection(config = {}) {
  ...
  if (groupSelect) {
    groupSelect.innerHTML = historicalGroupSelectOptions(requestedLang, resolved.group_key || "", country);
    groupSelect.value = String(resolved.group_key || "").trim();
  }
  if (countrySelect) {
    countrySelect.innerHTML = historicalAffiliationSelectOptions(requestedLang, resolved.group_key || "", country);
...
function homeSongSecondaryText(song = {}) {
  const rawSnippet = String(song?.snippet || "").replace(/\s+/g, " ").trim();
  if (rawSnippet) return rawSnippet;
  const titleNormalized = String(song?.title || "").trim().toLowerCase();
  const lines = splitLyricsLines(String(song?.lyrics || ""))
  ...
  return preview || String(song?.subtitle || "").trim();
}
...
function renderHomeSongCard(song = {}, options = {}) {
  ...
  const secondaryText = homeSongSecondaryText(song);
  ...
  return `
    <a class="yt-card ${flagUrl ? "yt-card-has-flag" : ""}" href="#/song/${encodeURIComponent(song.id)}"${cardStyle}>
      ...
      <div class="yt-card-content">
        ...
        ${secondaryText ? `<div class="yt-card-performer">${esc(secondaryText)}</div>` : ``}
...
function homeUI(data, params, homeExtras = {}) {
    ...
    ? Array.from(selectedLangBucket.countries.entries())
      .map(([countryKey, totalCount]) => {
        ...
        return {
          ...
          wallBg: wallpaperUrl,
          flagBg: flagUrl,
          groupKey: affiliationMeta?.group_key || "misc_world",
        };
      })
      .sort((a, b) => (Number(b.count) || 0) - (Number(a.count) || 0) || a.label.localeCompare(b.label, localeForSort))
    : [];
  const selectedLangCountrySections = selectedLangCountryCards.reduce((acc, item) => {
    const groupKey = String(item.groupKey || "misc_world").trim() || "misc_world";
    const last = acc[acc.length - 1];
    if (!last || last.groupKey !== groupKey) {
      acc.push({
        groupKey,
        title: getHistoricalGroupLabel(groupKey, uiLocale()),
        items: [item],
      });
      return acc;
    }
  ...
  const selectedLanguageCountryCardsHtml = selectedLangCountrySections.length
    ? `<div class="home-country-group-list">${selectedLangCountrySections.map((section) => `
      <section class="home-country-group-block">
        <div class="home-country-group-title">${esc(section.title)}</div>
  ...
  return `
    <section class="yt-main home-main">
      ...
      <div class="yt-chips ${showAdvanced ? "is-open" : "hidden"}" id="yt_advancedPanel">
        ...
        <label class="yt-chip-input">
          <span>${esc(historicalGroupFieldLabel())}</span>
          <select class="select" id="yt_group">${historicalGroupSelectOptions(lang || countrySelection.lang || "", countrySelection.group_key || "", country || "")}</select>
        </label>
        <label class="yt-chip-input">
          <span>${esc(historicalAffiliationFieldLabel())}</span>
          <select class="select" id="yt_country">${historicalAffiliationSelectOptions(lang || countrySelection.lang || "", countrySelection.group_key || "", country || "")}</select>
        </label>
        `}
    ...
    <div class="card request-card">
      ...
      <form id="requestForm" class="list request-form request-form-vertical">
        <div class="request-section">
          ...
          <label class="field"><div class="fieldLabel">${esc(historicalGroupFieldLabel())}</div><select class="select" id="rq_country_group">${historicalGroupSelectOptions("", "", "")}</select></label>
      ...
      <div class="card ac-editor-card ac-editor-page request-card song-edit-unified" id="ac_editor">
        ...
        <div class="request-section ac-editor-meta-section">
          ...
          <div class="ac-historical-grid ac-meta-locale-grid">
            ...
            <label class="field"><div class="fieldLabel">${esc(historicalGroupFieldLabel())}</div><select id="ac_country_group" class="select">${historicalGroupSelectOptions(song.lang || initialAffiliation.lang || "", initialAffiliation.group_key || "", song.country || "")}</select></label>
            <label class="field"><div class="fieldLabel">${esc(historicalAffiliationFieldLabel())} *</div><select id="ac_country" class="select" required>${historicalAffiliationSelectOptions(song.lang || initialAffiliation.lang || "", initialAffiliation.group_key || "", song.country || "")}</select></label>
          </div>
    ...
    <div>
      ...
      <div class="card ac-editor-card ab-editor">
        ...
        <div class="ac-historical-grid">
          ...
          <label class="field">
            <div class="fieldLabel">${esc(historicalGroupFieldLabel())}</div>
            <select class="select" id="ab_group">${historicalGroupSelectOptions("", "", "")}</select>
          </label>
          <label class="field">
...
function bindHistoricalSelectors(config = {}) {
  ...
  const getResolvedGroup = (countryValue = "") => {
    const explicitGroup = String(groupSelect.value || "").trim();
    if (explicitGroup) return explicitGroup;
    return resolveHistoricalAffiliationSelection(countryValue).group_key || "";
  };

  const renderGroups = (selectedGroup = "", currentCountry = "") => {
    groupSelect.innerHTML = historicalGroupSelectOptions(getLangValue(), selectedGroup, currentCountry);
  };
  const renderCountries = (selectedCountry = "", selectedGroup = "") => {
    countrySelect.innerHTML = historicalAffiliationSelectOptions(getLangValue(), selectedGroup, selectedCountry);
  };
  ...
  const refresh = (options = {}) => {
    ...
    const nextGroup = String(options.group || getResolvedGroup(selectedCountry) || resolved.group_key || "").trim();
    renderGroups(nextGroup, selectedCountry);
    if (nextGroup) groupSelect.value = nextGroup;
    renderCountries(selectedCountry, nextGroup);
    if (selectedCountry) countrySelect.value = selectedCountry;
    else if (!countrySelect.value) countrySelect.value = "";
    config.onRefresh?.({ lang: getLangValue(), group: groupSelect.value || "", country: countrySelect.value || "" });
  };
  ...
  groupSelect.addEventListener("change", () => {
    ...
    renderCountries(currentCountry, groupSelect.value || "");
    if (currentCountry) {
      const meta = getHistoricalAffiliationMeta(currentCountry);
      if (meta?.group_key === (groupSelect.value || "")) {
        countrySelect.value = currentCountry;
      }
    }
    config.onGroupChange?.({ lang: getLangValue(), group: groupSelect.value || "", country: countrySelect.value || "" });
  });

  countrySelect.addEventListener("change", () => {
    const currentCountry = getCountryValue();
    const resolved = resolveHistoricalAffiliationSelection(currentCountry);
    if (resolved.group_key && groupSelect.value !== resolved.group_key) {
      groupSelect.value = resolved.group_key;
      renderCountries(currentCountry, resolved.group_key);
      if (currentCountry) countrySelect.value = currentCountry;
    }
    config.onCountryChange?.({ lang: getLangValue(), group: groupSelect.value || "", country: currentCountry });
  });

  langSelect?.addEventListener("change", () => {
    refresh();
    config.onLangChange?.({ lang: getLangValue(), group: groupSelect.value || "", country: getCountryValue() });
  });

  refresh({
    country: normalizeSongCountry(config.initialCountry || countrySelect.value || "") || "",
    group: String(config.initialGroup || groupSelect.value || "").trim(),
  });
...
function bindAdminHistoricalVisuals(ctx) {
  ...
  const binder = bindHistoricalSelectors({
    ...
    onGroupChange: ({ country }) => fillEditor(country),
...
export function bind(route, ctx) {
  ...
  if (route.name === "home") {
    ...
    qs("yt_resetAdvanced")?.addEventListener("click", () => {
      ...
      if (!lockedCountry) {
        homeHistoricalBinder.refresh({ group: "", country: "" });
      }
  ...
  if (route.name === "request") {
    ...
    const requestHistoricalBinder = !isFragmentReport
      ? bindHistoricalSelectors({
          ...
          onGroupChange: () => syncPeriodVisibility("rq_country", "rq_period_wrap", "rq_period"),
    ...
    if (!isFragmentReport) {
      ...
      requestHistoricalBinder?.refresh({ group: "", country: "" });
  ...
  if ((route.name === "admin" && ctx.section === "editor") || route.name === "draft") {
    ...
    const editorHistoricalBinder = bindHistoricalSelectors({
      ...
      onGroupChange: () => syncPeriodVisibility("ac_country", "ac_period_wrap", "ac_period"),
