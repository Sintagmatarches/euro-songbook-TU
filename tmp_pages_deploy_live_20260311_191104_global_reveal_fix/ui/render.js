...
function buildHomeSongSecondaryPreview(song = {}, options = {}) {
  ...
  for (let index = 0; index < sourceLines.length; index += 1) {
    ...
    if (!normalized || normalized === titleNormalized || /^[\[(].*[\])]$/.test(line)) continue;
  ...
  return subtitle
    ? { parts: [{ type: "line", text: subtitle }], truncated: false }
  ...
  if (route.name === "home") {
    ...
    if (params.searched === "1" && progressiveFeed) {
      ...
      if (!queue.length) {
        ...
        if (typeof window.__applyStagedReveal === "function") {
          window.__applyStagedReveal(progressiveFeed);
        }
        progressiveLoadingScreen?.classList.add("hidden");
        window.requestAnimationFrame(() => {
