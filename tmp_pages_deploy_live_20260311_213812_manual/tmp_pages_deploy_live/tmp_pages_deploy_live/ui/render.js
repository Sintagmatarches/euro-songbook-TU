...
function renderStructuredLyrics(rawLyrics, options = {}) {
  ...
  if (!parsed.blocks.length) {
    ...
    return {
      html: fallbackText
        ? `<div class="song-lyric-block is-verse"><pre class="lyrics song-primary-lyrics song-text song-lyric-text">${renderLines.length ? renderEntriesWithConfidence(fallbackEntries) : renderWithUnknownMarks(fallbackText)}</pre><span class="song-lyric-marker song-lyric-marker-empty" aria-hidden="true"></span></div>`
        : "",
  ...
  const html = parsed.blocks.map((block) => {
    ...
    return `
      <div class="${classes}">
        <pre class="lyrics song-primary-lyrics song-text song-lyric-text">${renderLines.length ? renderEntriesWithConfidence(block.lines || []) : renderWithUnknownMarks(block.text)}</pre>
