...
function buildSnippet(song = {}, queryTokens = []) {
  ...
  for (const line of lines) {
    ...
    if (!normalizedTokens.some((token) => normalizedLine.includes(token))) continue;
    return line;
  }
  return lines[0];
}
...
function computeMatchMetadata(song = {}, analysis = {}, candidateTermsByField = {}) {
  ...
  const titleTerms = new Set(candidateTermsByField.title || fieldSequence(song?.title || ""));
  const subtitleTerms = new Set(candidateTermsByField.subtitle || fieldSequence(song?.subtitle || ""));
  const lyricsTerms = new Set(candidateTermsByField.lyrics || fieldSequence(song?.lyrics || ""));
