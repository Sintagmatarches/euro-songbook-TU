...
function renderHomeSongSecondary(song = {}) {
  const preview = buildHomeSongSecondaryPreview(song, { maxLines: 3 });
  ...
  return preview.parts.map((part, index) => {
    ...
    return `<span class="yt-card-performer-line">${esc(withEllipsis)}</span>`;
  }).join("");
}
...
function renderHomeSongCard(song = {}, options = {}) {
  ...
  const searchBadge = song?.match_bucket ? searchBucketLabel(song.match_bucket) : "";
    ...
    <a class="yt-card ${flagUrl ? "yt-card-has-flag" : ""}" href="#/song/${encodeURIComponent(song.id)}"${cardStyle}>
      ...
      <div class="yt-card-content">
        <div class="yt-card-title-row">
          ...
          ${searchBadge ? `<span class="yt-card-search-badge">${esc(searchBadge)}</span>` : ``}
  ...
  if (route.name === "home") {
    ...
    if (params.searched === "1" && progressiveFeed) {
      ...
      if (!queue.length) {
        ...
        const cardsHtml = renderHomeResultsFeedItems(queue, { backgroundsByCountry, flagDevice });
        window.setTimeout(() => {
