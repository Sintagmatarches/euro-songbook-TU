...
function clampConfidencePercent(value, fallback = 100) {
  const parsed = Number.parseFloat(String(value ?? fallback));
  if (!Number.isFinite(parsed)) {
    const fallbackParsed = Number.parseFloat(String(fallback ?? 100));
    if (!Number.isFinite(fallbackParsed)) return 100;
    return Math.max(0, Math.min(100, Math.round(fallbackParsed)));
  }
...
export function normalizeConfidenceValue(value, fallback = 100) {
  if (value && typeof value === "object") {
    if (value.confidence !== undefined) return normalizeConfidenceValue(value.confidence, fallback);
    if (value.level !== undefined) return normalizeConfidenceValue(value.level, fallback);
    if (value.confidence_level !== undefined) return normalizeConfidenceValue(value.confidence_level, fallback);
  }

  const normalized = String(value ?? "").trim().toLowerCase();
  if (CONFIDENCE_LEVEL_MAP.has(normalized)) {
    return confidenceScoreFromLevel(normalized, fallback);
  }

  const numeric = Number.parseFloat(String(value ?? ""));
  if (Number.isFinite(numeric)) return clampConfidencePercent(numeric, fallback);

  return clampConfidencePercent(fallback, 100);
}
...
export function normalizeConfidenceSegment(segment, options = {}) {
  ...
  const fallbackLevel = options?.fallbackLevel || "sure";
  const confidenceSource = segment.confidence ?? segment.score ?? segment.value;
  const confidence = confidenceSource !== undefined && Number.isFinite(Number.parseFloat(String(confidenceSource)))
    ? clampConfidencePercent(confidenceSource, confidenceScoreFromLevel(fallbackLevel))
    : normalizeConfidenceValue(
      segment.level ?? segment.confidence_level ?? confidenceSource,
      confidenceScoreFromLevel(fallbackLevel),
    );
  const level = confidenceLevelFromValue(confidence, fallbackLevel);
  if (options?.dropExact === true && confidence >= 100) return null;
  return {
    start,
    end,
    level,
...
function mergeAdjacentSegments(segments = []) {
  ...
  for (const item of ordered) {
    ...
    if (
      previous
      && Number(previous.confidence || 0) === Number(current.confidence || 0)
      && current.start <= previous.end
    ) {
      previous.end = Math.max(previous.end, current.end);
      previous.level = confidenceLevelFromValue(previous.confidence, previous.level || "sure");
