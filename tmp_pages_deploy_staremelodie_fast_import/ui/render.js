...
function searchBucketLabel(bucket = "") {
  ...
  if (safe === "exact") {
    if (uiLocale() === "ru") return "Точные совпадения";
    if (uiLocale() === "uk") return "Точні збіги";
    if (uiLocale() === "et") return "Täpsed vasted";
...
function renderSearchBucketSummary(bucketCounts = {}) {
  const ordered = ["exact", "partial", "text", "fuzzy"];
  const chips = ordered
    .map((bucket) => ({
      bucket,
      count: Math.max(0, Number(bucketCounts?.[bucket] || 0)),
    }))
    .filter((entry) => entry.count > 0)
    .map((entry) => `<span class="home-results-bucket" data-bucket="${esc(entry.bucket)}">${esc(searchBucketLabel(entry.bucket))}: ${esc(entry.count)}</span>`)
    .join("");
  if (!chips) return "";
  return `<div class="home-results-buckets">${chips}</div>`;
}
...
function renderHomeSongCard(song = {}, options = {}) {
  ...
  const metaParts = [formatLang(song.lang), formatCountry(song.country)].map((value) => String(value || "").trim()).filter(Boolean);
  const metaRow = `
    <div class="yt-card-meta-row yt-card-meta-row-side">
      <span class="yt-card-line">${esc(metaParts.join(" · "))}</span>
    </div>
  `;
  const yearBadge = song.year ? `<span class="yt-card-year">${esc(song.year)}</span>` : ``;
  const sideRail = `
    <div class="yt-card-side ${hasFlag ? "yt-card-side-flag" : "yt-card-side-plain"}">
      <div class="yt-card-side-footer">
        ${metaRow}
        ${yearBadge}
      </div>
    </div>
  `;
  return `
    <a class="yt-card ${hasFlag ? "yt-card-has-flag" : ""} yt-card-preview-${previewLineLimit}" href="#/song/${encodeURIComponent(song.id)}"${cardStyle}>
...
function renderHomeResultsFeedItems(items = [], options = {}) {
  ...
  return groups.map((group) => `
    <section class="home-result-group" data-bucket="${esc(group.bucket)}">
      <div class="home-result-group-title">${esc(searchBucketLabel(group.bucket))}</div>
...
function homeUI(data, params, homeExtras = {}) {
  ...
  const languageCards = Array.from(langCountryMap.entries())
    .map(([langKey, bucket]) => {
      ...
      const flagUrl = topCountry
        ? resolveVisualSymbolUrl(background?.visual_profile || {}, {
            kind: "square",
            device: "desktop",
          })
        : "";
  ...
  const selectedLangCountryCards = selectedLangBucket
    ? Array.from(selectedLangBucket.countries.entries())
      .map(([countryKey, totalCount]) => {
        ...
        const wallpaperUrl = String(background?.desktop_image_url || background?.mobile_image_url || "").trim();
        const affiliationMeta = getHistoricalAffiliationMeta(countryKey);
        return {
          key: countryKey,
          ...
          wallBg: wallpaperUrl,
          flagBg: flagUrl,
          sortOrder: Number(affiliationMeta?.sort_order || 999999),
        };
      })
      .sort((a, b) => b.sortOrder - a.sortOrder || a.label.localeCompare(b.label, localeForSort))
    : [];
  ...
  const languageCardsHtml = languageCards.length
    ...
    item.flagBg ? `--home-country-flag:${toCssUrlValue(item.flagBg)}` : "",
      ...
      >
        ${renderHomeCountryCardLabel(item.label)}
  ...
  const selectedLanguageCountryCardsHtml = selectedLangCountryCards.length
    ? `<div class="home-country-group-grid">
  ...
  const bucketSummaryMarkup = renderSearchBucketSummary(data?.bucket_counts || {});
    ...
    <div class="card request-card">
      ...
      <form id="requestForm" class="list request-form request-form-vertical">
        <div class="request-section">
          ...
          <label class="field"><div class="fieldLabel">${esc(historicalAffiliationFieldLabel())}</div><select class="select" id="rq_country" required>${historicalAffiliationSelectOptions("", "", "")}</select></label>
...
function adminTabs(active) {
  ...
  return `
    <div class="admin-tabs" style="margin-bottom:12px">
      ${(canEditSongs || canBulkImport) ? `<a class="btn admin-tab-btn ${active === "content" ? "primary" : "ghost"}" href="#/admin/content">${esc(songsTabLabel)}</a>` : ``}
...
function adminContentUI(data, params) {
  ...
  const listItems = items.map((s) => {
    ...
    const previewHref = status === "published"
      ...
      : "";
    const editHref = `#/admin/editor?id=${encodeURIComponent(s.id)}`;
    const metaPrimary = [
      formatLang(s.lang),
    ...
    return `
      <div class="songCard ac_item ${isRecent ? "is-recent" : ""}">
        ${previewHref
          ? `<a class="ac_item_main" href="${esc(previewHref)}">`
          : `<div class="ac_item_main">`}
          <div class="ac_item_preview ${isRecent ? "is-recent" : ""}">
            <div class="songTitle">${esc(s.title)}</div>
            <div class="songMeta">${esc(metaPrimary)}</div>
          </div>
        ${previewHref ? `</a>` : `</div>`}
        <div class="ac_item_side">
          ${badge(status)}
          <div class="ac_item_actions">
            ${previewHref ? `<a class="btn ghost ac_item_action" href="${esc(previewHref)}">${esc(adminContentPreviewLabel())}</a>` : ``}
            <a class="btn ghost ac_item_action" href="${esc(editHref)}">${esc(adminContentEditLabel())}</a>
          </div>
        </div>
      </div>
    `;
  }).join("");
  const bulkImportCard = !canBulkImport ? "" : `
  ...
  `;
  const readOnlyHint = (!canEditSongs && canBulkImport)
    ? `<div class="card"><div class="muted">${esc(bulkImportUiText("readOnlyList"))}</div></div>`
    : "";
  return `
    <div class="ac-content-stack">
      ${adminTabs("content")}
      ${bulkImportCard}
      ${listCard || readOnlyHint}
    </div>
  `;
}
      ...
      <div class="card ac-editor-card ac-editor-page request-card song-edit-unified${requestMode ? " ac-editor-request-mode" : ""}" id="ac_editor">
        ...
        <div class="request-section ac-editor-meta-section">
          ...
          <div class="ac-historical-grid ac-meta-locale-grid">
            ...
            <label class="field"><div class="fieldLabel">${esc(historicalAffiliationFieldLabel())} *</div><select id="ac_country" class="select" required>${historicalAffiliationSelectOptions(song.lang || initialAffiliation.lang || "", "", song.country || "")}</select></label>
          </div>
...
async function standardizeFlagImageHorizontalCrop(sourceUrl, width, height) {
  ...
  const croppedBounds = findOpaqueImageBounds(sourceContext.getImageData(0, 0, sourceWidth, sourceHeight));
  ...
  // Narrow or square stickers are placed on a transparent long canvas instead of being rejected.
  const outerPadX = Math.round(targetWidth * (targetWidth >= 3000 ? 0.055 : 0.06));
  const outerPadY = Math.round(targetHeight * 0.1);
  const maxDrawWidth = Math.max(1, Math.round(targetWidth * (targetWidth >= 3000 ? 0.28 : 0.36)));
  ...
  ctx.shadowBlur = Math.max(8, Math.round(targetHeight * 0.04));
  ctx.shadowOffsetX = Math.max(4, Math.round(targetHeight * 0.012));
  ctx.shadowOffsetY = Math.max(6, Math.round(targetHeight * 0.018));
  ctx.drawImage(
    sourceCanvas,
    croppedBounds.left,
    croppedBounds.top,
...
async function normalizeVisualSymbolLongImages(symbol = {}) {
  ...
  return {
    ...
    long: normalized.long
      ? await standardizeFlagImageHorizontalCrop(normalized.long, FLAG_CARD_STANDARDS.desktopLong.width, FLAG_CARD_STANDARDS.desktopLong.height)
      : "",
    long_mobile: normalized.long_mobile
      ? await standardizeFlagImageHorizontalCrop(normalized.long_mobile, FLAG_CARD_STANDARDS.mobileLong.width, FLAG_CARD_STANDARDS.mobileLong.height)
      : "",
  };
}
...
export async function render(route) {
  ...
  if (route.name === "admin") {
    ...
    if (section === "content") {
      ...
      const data = canEditSongs
        ? await api.adminSongs({
          q: params.q || "",
      ...
      return {
        html: adminContentUI(data, { ...params, recent: params.recent || "", sort: params.sort || "newest" }),
        ctx: { section, data, params: { ...params, recent: params.recent || "", sort: params.sort || "newest" }, canEditSongs, canBulkImport },
      };
    }
    ...
    if (bulkText && bulkPreview && bulkStats && bulkSubmit) {
      ...
      bulkSubmit.addEventListener("click", async () => {
        ...
        try {
          ...
          removedSongIndexes = new Set();
          renderBulkPreview();
          if (qs("ac_q") || qs("ac_search")) {
            location.hash = buildHash({ page: 1 });
          }
        } catch (error) {
          showStatusOverlay(`${t("common.error")}: ${error?.message || t("common.error")}`, "error");
        } finally {
